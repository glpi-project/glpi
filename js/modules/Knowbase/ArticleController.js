...
/* global glpi_ajax_dialog, glpi_alert, glpi_confirm_danger, glpi_toast_error, glpi_toast_info, */
...
export class GlpiKnowbaseArticleController
{
    ...
    #updateTranslationsCount()
    {
        ...
        if (count_el) {
            ...
            count_el.textContent = count === 1
                ? `${count} ${__('translation')}`
                : `${count} ${__('translations')}`;
        }
    }
    ...
    #moveOptionToExistingGroup(language_code)
    {
        ...
        const option = select.querySelector(`option[value="${language_code}"]`);
        if (!option) {
            return;
        }

        let existing_group = select.querySelector(`optgroup[label="${__('Existing translations')}"]`);
        if (!existing_group) {
        ...
        const new_group = select.querySelector(`optgroup[label="${__('Add new translation')}"]`);
    ...
    #moveOptionToNewGroup(language_code)
    {
        ...
        let new_group = select.querySelector(`optgroup[label="${__('Add new translation')}"]`);
        if (!new_group) {
        ...
        const existing_group = select.querySelector(`optgroup[label="${__('Existing translations')}"]`);
