...
HTML;
    ...
    public static function redirectProvider(): iterable
    {
        ...
        foreach (['helpdesk', 'central'] as $interface) {
            ...
            yield [
                'interface' => $interface,
                'where'     => 'http://localhost:8088/',
                'result'    => 'http://localhost:8088/',
            ];
            yield [
                'interface' => $interface,
                'where'     => 'http://localhost:8088/front/computer.php?id=15',
                'result'    => 'http://localhost:8088/front/computer.php?id=15',
            ];
    ...
    public function testComputeRedirect(string $interface, string $where, ?string $result): void
    {
        ...
        $this->assertSame($result, $this->callPrivateMethod($instance, 'computeRedirect', $where));
    }
}
