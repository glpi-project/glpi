...
class APIRestTest extends APIBaseClass
{
    ...
    /**
     * Test that UID collisions are resolved correctly with uid_cols parameter
     *
     * @tags   api
     * @covers API::searchItems
     * @covers API::listSearchOptions
     */
    public function testUidColsCollisionResolution()
    {
        // Debug: check content-type
        $res = $this->doHttpRequest('GET', 'initSession/', ['auth' => [TU_USER, TU_PASS]]);
        $contentType = $res->getHeader('content-type');
        echo "\nActual content-type: " . print_r($contentType, true);

        $headers = ['Session-Token' => $this->session_token];

        // Create a ticket with some SLA fields expired for testing
        $ticket = new \Ticket();
        $tickets_id = $ticket->add([
            'name' => 'Test ticket for UID collision',
            'content' => 'Test ticket content',
            'entities_id' => getItemByTypeName('Entity', '_test_root_entity', true),
            'status' => \CommonITILObject::INCOMING,
            // Set some dates to potentially trigger SLA calculations
            'date' => date('Y-m-d H:i:s', strtotime('-1 day')),
            'time_to_resolve' => date('Y-m-d H:i:s', strtotime('-1 hour')), // Expired TTO
        ]);
        $this->assertGreaterThan(0, $tickets_id);

        // Test search with uid_cols=true and SLA fields that historically had collisions
        $data = $this->query(
            'search/Ticket',
            [
                'headers' => $headers,
                'query'   => [
                    'uid_cols'       => 'true',
                    'forcedisplay'   => [82, 159, 182, 187], // SLA expired fields
                    'criteria'       => [
                        [
                            'field'      => 1, // ID
                            'searchtype' => 'equals',
        ...
        if (!empty($data['data'])) {
            ...
            // Check that all SLA UID fields are present and unique
            $sla_uids = [
                'Ticket.is_late.82',   // Time to resolve exceeded
                'Ticket.is_late.159',  // Time to own exceeded
                'Ticket.is_late.182',  // Internal time to resolve exceeded
                'Ticket.is_late.187',  // Internal time to own exceeded
            ];

            $found_uids = [];
            foreach ($sla_uids as $uid) {
                if (array_key_exists($uid, $first_row)) {
                    $found_uids[] = $uid;
                }
            }

            // We should have found some SLA UIDs (at least the ones that are configured)
            $this->assertGreaterThan(
                0,
                count($found_uids),
                'No SLA UID fields found in response. Available keys: ' . implode(', ', array_keys($first_row))
            );

            // Check that all found UIDs are unique (no duplicates)
            $this->assertEquals(
                count($found_uids),
                count(array_unique($found_uids)),
                'Duplicate UID keys found: ' . implode(', ', $found_uids)
            );

            // Verify that the UIDs follow the expected pattern with appended search option ID
            foreach ($found_uids as $uid) {
                $this->assertMatchesRegularExpression(
                    '/^Ticket\.is_late\.\d+$/',
                    $uid,
                    "UID '$uid' doesn't follow expected pattern 'Ticket.is_late.{searchOptionId}'"
                );
            }
        }
    ...
    /**
     * Test that listSearchOptions returns unique UIDs for SLA fields
     *
     * @tags   api
     * @covers API::listSearchOptions
     */
    public function testListSearchOptionsUniqueUids()
    {
        $headers = ['Session-Token' => $this->session_token];
        ...
        $this->assertIsArray($data);
        unset($data['headers']); // Remove HTTP headers from response

        // Collect all UIDs that contain 'is_late'
        $is_late_uids = [];
        foreach ($data as $search_id => $search_option) {
            if (is_array($search_option) && isset($search_option['uid']) && strpos($search_option['uid'], 'is_late') !== false) {
                $is_late_uids[$search_id] = $search_option['uid'];
            }
        }

        // We should have multiple SLA fields with is_late in their UID
        $this->assertGreaterThan(
            1,
            count($is_late_uids),
            'Expected multiple SLA fields with is_late UIDs, found: ' . json_encode($is_late_uids)
        );

        // Check that all UIDs are unique
        $unique_uids = array_unique(array_values($is_late_uids));
        $this->assertEquals(
            count($is_late_uids),
            count($unique_uids),
            'Found duplicate UIDs in is_late fields: ' . json_encode($is_late_uids)
        );

        // Verify expected SLA search option IDs have correct UIDs
        $expected_sla_ids = [82, 159, 182, 187];
        foreach ($expected_sla_ids as $expected_id) {
            if (isset($is_late_uids[$expected_id])) {
                $expected_uid = "Ticket.is_late.$expected_id";
                $this->assertEquals(
                    $expected_uid,
                    $is_late_uids[$expected_id],
                    "Search option $expected_id should have UID '$expected_uid', got '{$is_late_uids[$expected_id]}'"
                );
            }
        }
    }
