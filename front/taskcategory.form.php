...
if (isset($_POST["add"])) {
   ...
   logEvent($_POST["id"], "taskcategory", 4, "setup", $_SESSION["glpiname"]." ".$LANG['log'][22]);
   glpi_header($CFG_GLPI["root_doc"]."/front/taskcategory.php");

} else if (isset($_POST["update"])) {
   $category->check($_POST["id"],'w');
   $category->update($_POST);

   logEvent($_POST["id"], "taskcategory", 4, "setup", $_SESSION["glpiname"]." ".$LANG['log'][21]);
   glpi_header($_SERVER['HTTP_REFERER']);

} else {
   commonHeader($LANG['common'][12],$_SERVER['PHP_SELF'],"config","taskcategory");
