...
$fup = new ChangeFollowup();

if (isset($_POST["add"])) {
   $fup->check(-1, CREATE, $_POST);
   $fup->add($_POST);

   Event::log($fup->getField('changes_id'), "change", 4, "tracking",
   ...
   $ticket = new Ticket();
   if ($ticket->getFromDB($_POST["changes_id"]) && $ticket->canApprove()) {
      $fup->add($_POST);
   ...
   Html::redirect(Toolbox::getItemTypeFormURL('Change')."?id=".$fup->getField('changes_id'));
