...
$NEEDED_ITEMS = array ('entity', 'group', 'ldap', 'profile', 'rulesengine', 'rule.right', 'setup',
                       'user');
...
if (isset ($_GET['next'])) {
   ...
   if (isset ($_SESSION["ldap_sync"])) {
      if ($count = count($_SESSION["ldap_sync"])) {
         $percent = min(100, round(100 * ($_SESSION["ldap_sync_count"] - $count) /
