...
$NEEDED_ITEMS = array('computer', 'document', 'group', 'monitor', 'networking', 'peripheral',
                      'phone', 'printer', 'software', 'supplier', 'tracking', 'user');
...
if (!isset($_GET["group"]) || isset($_GET['reset'])) {
   $_GET["group"] = 0;
}
...
if (!haveRight("show_all_ticket","1") && !haveRight("show_assign_ticket",'1')) {
   searchSimpleFormTracking($_GET["extended"],$_SERVER['PHP_SELF'],$_GET["status"],$_GET["tosearch"],
                            $_GET["search"],$_GET["group"],$_GET["showfollowups"],
                            $_GET["ticketcategories_id"]);
   showTrackingList($_SERVER['PHP_SELF'],$_GET["start"],$_GET["sort"],$_GET["order"],$_GET["status"],
                    $_GET["tosearch"],$_GET["search"],$_SESSION["glpiID"],$_GET["group"],
