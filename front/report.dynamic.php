...
if (isset($_GET["item_type"])&&isset($_GET["display_type"])){
	...
	switch ($_GET["item_type"]){
		case KNOWBASE_TYPE :
			showKbItemList($_SERVER['PHP_SELF'],$_GET["contains"],$_GET["start"],$_GET["knowbaseitemscategories_id"],$_GET["faq"]);
			break;
		case TRACKING_TYPE :
			showTrackingList($_SERVER['PHP_SELF'],$_GET["start"],$_GET["sort"],$_GET["order"],$_GET["status"],$_GET["tosearch"],$_GET["search"],$_GET["users_id"],$_GET["group"],$_GET["showfollowups"],$_GET["ticketscategories_id"],$_GET["users_id_assign"],$_GET["suppliers_id_assign"],$_GET["groups_id_assign"],$_GET["priority"],$_GET["request_type"],$_GET["items_id"],$_GET["itemtype"],$_GET["field"],$_GET["contains"],$_GET["date1"],$_GET["date2"],$_GET["only_computers"],$_GET["enddate1"],$_GET["enddate2"],$_GET["datemod1"],$_GET["datemod2"]);
			break;
		case STAT_TYPE :
