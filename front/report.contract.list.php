...
if (isset($_POST["item_type"])&&is_array($_POST["item_type"])){
	...
	if (in_array($val,$items)){
      $itemtable=getTableForItemType($val);
		...
		$query[$val] = "SELECT  $itemtable.name AS itemname, $itemtable.is_deleted AS itemdeleted, ";
		...
		$query[$val].= " FROM glpi_contracts_items ";
		$query[$val].= " INNER JOIN glpi_contracts ON (glpi_contracts_items.contracts_id=glpi_contracts.id) ";
		$query[$val].= " INNER JOIN $itemtable ON (glpi_contracts_items.itemtype='$val' AND $itemtable.id =  glpi_contracts_items.items_id )";
		$query[$val].= " LEFT JOIN glpi_infocoms ON ( glpi_infocoms.itemtype = '$val' AND $itemtable.id =  glpi_infocoms.items_id ) ";
		$query[$val].= " LEFT JOIN glpi_contracttypes ON (glpi_contracts.contracttypes_id = glpi_contracttypes.id) ";
		$query[$val].= " LEFT JOIN glpi_locations ON ($itemtable.locations_id = glpi_locations.id) ";
		$query[$val].= " LEFT JOIN glpi_entities ON ($itemtable.entities_id = glpi_entities.id) ";

		$query[$val].=" WHERE $itemtable.is_template ='0' ";
		$query[$val].=getEntitiesRestrictRequest("AND",$itemtable);
