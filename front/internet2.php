...
echo "&nbsp;<input type='submit' name='search' value='"._sx('button', 'Search')."' class='submit'>";
echo "</form>";
...
if (isset($_POST['search'])) {
   ...
   if (isset($items)) {
      ...
      if ($allow_wildcard) {
         printf(__('%1$s (%2$s)'), $msg, __('allow wildcards'));
      } else {
         printf(__('%1$s (%2$s)'), $msg, __('exact match'));
      }
      echo "</th></tr>";
      foreach ($items as $item) {
         ...
         printf(__('%1$s: %2$s'), $type, implode(' &gt; ', $elements_to_display));
         echo "</td></tr>";
      }
   }
}
