...
; 1.1 (Fri May  6 17:25:00 CEST 2005)
;  + clean file using the empty dump  (Julien Dombre - GLPI development Team)

; NOTE
; This is a quick script that import most of the data, but there is some
; differences from IRM to GLPI because GLPI came from IRMA, an IRM 1.0.X
; fork.
; Particulary expect trouble on licence handling.
;
; Also, i have not even try to convert some data, as templates and
; configuration data...
...
; 2) install GLPI 0.51, follow the installation instructions, i suppose to
;  install to the glpipp database
;  use the empty dump of GLPI

; 3) restore the irm database into the glpi database, there is no table name
...
; table fasttracktemplates not used in glpi... i do not think will be
...
; setup users and prefs, i do not know if password work, but could be.
...
; discarded by mysql, so i have not bother...
;
