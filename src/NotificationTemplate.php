...
class NotificationTemplate extends CommonDBTM
{
    ...
    public function getTemplateByLanguage(
        ...
        // Include user-specific identifier in the final template ID to ensure each user gets unique template data
        $user_identifier = '';
        ...
        if (!isset($this->templates_by_languages[$tid])) {
            ...
            // Now generate user-specific template by applying user-specific data to the base template
            if (isset($this->templates_by_languages[$base_template_key])) {
                $this->templates_by_languages[$tid] = $this->applyUserSpecificData(
                    $this->templates_by_languages[$base_template_key],
                    $target,
                    $user_infos,
                    $event,
