   ...
   //Empty value displayed in a dropdown
    ...
    public static function getDropdownComments(string $table, int $id, bool $translate = true, bool $tooltip = true)
    {
        ...
        if (
            ...
            $criteria['LEFT JOIN'] = [
                'glpi_dropdowntranslations AS comment_translations' => [
                    'ON' => [
        ...
        if (
            ...
            $criteria['LEFT JOIN'] = [
                'glpi_dropdowntranslations AS completename_translations' => [
                    'ON' => [
        ...
        if ($tooltip) {
            ...
            if (is_a($itemtype, CommonTreeDropdown::class, true)) {
                $comment_rows[] = sprintf(
                    ...
                    '</span>' . htmlspecialchars($data['translated_completename'] ?: $data['completename'])
                );
            }
            ...
            switch ($itemtype) {
                case Contact::class:
                    ...
                    foreach ($fields as $key => $label) {
                        if (!empty($data[$key])) {
                            $comment_rows[] = sprintf(
                                __s('%1$s: %2$s'),
                                '<span class="b">' . htmlspecialchars($label),
                                '</span>' . htmlspecialchars($data[$key])
                            );
                        }
                    }
                    break;
                ...
                case Budget::class:
                    if (!empty($data['locations_id'])) {
                        $comment_rows[] = sprintf(
                            __s('%1$s: %2$s'),
                            '<span class="b">' . htmlspecialchars(Location::getTypeName(1)),
                            '</span>' . htmlspecialchars(self::getDropdownName('glpi_locations', $data['locations_id'], translate: $translate))
                        );
                    }
                    if (!empty($data['budgettypes_id'])) {
                        $comment_rows[] = sprintf(
                            ...
                            '</span>' . htmlspecialchars(self::getDropdownName('glpi_budgettypes', $data['budgettypes_id'], translate: $translate))
                        );
                    }
                ...
                case Location::class:
                    if (!empty($data['alias'])) {
                        $comment_rows[] = sprintf(
                            ...
                            '</span>' . htmlspecialchars($data['alias'])
                        );
                    }
                    if (!empty($data['code'])) {
                        $comment_rows[] = sprintf(
                            ...
                            '</span>' . htmlspecialchars($data['code'])
                        );
                    }
                    $address_comment = '';
                    ...
                    if (!empty($address)) {
                        $address_comment .= htmlspecialchars($address);
                    }
                    if (!empty($address) && (!empty($town) || !empty($country))) {
                        $address_comment .= '<br/>';
                    }
                    if (!empty($town)) {
                        $address_comment .= htmlspecialchars($town);
                    }
                    if (!empty($country)) {
                        if (!empty($town)) {
                            $address_comment .= ' - ';
                        }
                        $address_comment .= htmlspecialchars($country);
                    }
        ...
        $comment .= nl2br(htmlspecialchars($data['translated_comment'] ?: $data['comment']));
