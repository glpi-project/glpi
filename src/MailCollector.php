...
class MailCollector extends CommonDBTM
{
    ...
    public function getBody(\Laminas\Mail\Storage\Message $message)
    {
        ...
        foreach ($parts as $part) {
            ...
            if ($content_type === 'text/html') {
                ...
                // First, try standard body extraction (recommended approach)
                $body_matches = [];
                $body_content = null;
                if (preg_match('/<body[^>]*>\s*(?<body>.+?)\s*<\/body>/is', $content, $body_matches) === 1) {
                    $body_content = $body_matches['body'];

                    // Check if body content is meaningful (not just GLPI markers or empty content)
                    $text_content = strip_tags($body_content);
                    $text_content = html_entity_decode($text_content, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                    $text_content = trim($text_content);

                    // Check if content is mostly GLPI markers that will be cleaned later
                    $is_mostly_glpi_markers = $this->isContentMostlyGlpiMarkers($text_content);

                    if (!empty($text_content) && strlen($text_content) > 10 && !$is_mostly_glpi_markers) {
                        $content = $body_content;
                    } else {
                        // Body exists but has minimal/GLPI content, try alternative extraction
                        $reply_content = $this->extractReplyContent($content);
                        $content = $reply_content !== null ? $reply_content : $body_content;
                    }
                } else {
                    // No body tag found, try alternative extraction as fallback
                    $reply_content = $this->extractReplyContent($content);
    ...
    /**
     * Check if content is mostly composed of GLPI markers that will be cleaned later
     *
     * @param string $text_content Plain text content to check
     * @return bool True if content is mostly GLPI markers
     */
    private function isContentMostlyGlpiMarkers(string $text_content): bool
    {
        // Common GLPI markers patterns
        $glpi_patterns = [
            '/=-=-=-=.*?Pour répondre.*?=-=-=-=/s',
            '/=_=_=_=.*?Pour répondre.*?=_=_=_=/s',
            '/Généré automatiquement par GLPI/i',
            '/To reply by email.*?write.*?this line/i',
            '/Automatically generated by GLPI/i',
        ];

        $original_length = strlen($text_content);
        $cleaned_content = $text_content;

        // Remove GLPI markers
        foreach ($glpi_patterns as $pattern) {
        ...
        // If after removing GLPI markers, less than 30% of original content remains,
        // consider it as mostly GLPI markers
        return $cleaned_length < ($original_length * 0.3);
    }
