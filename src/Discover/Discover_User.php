    ...
    public function getLocalizedConfig(): array
    {
        ...
        array_walk_recursive($discoverConfig, function (&$value, $key) {
            if ($key === 'content' && str_starts_with($value, 'file://')) {
                $filepath = GLPI_ROOT . '/vendor/glpi-project/lessons/translated/' . Session::getLanguage() . '/' . substr($value, 7);

                // If the file doesn't exist in the user language, we fallback to english
                if (!file_exists($filepath)) {
                    $filepath = GLPI_ROOT . '/vendor/glpi-project/lessons/translated/en_GB/' . substr($value, 7);
                }
