        ...
        foreach ($value as $key => $val) {
            ...
            if (isset($data['found_inventories'])) {
                ...
                if ($data['found_inventories'][0] == 0) {
                    ...
                    $items_id = $this->addItem($instance, $input, \Log::HISTORY_ADD_RELATION);
                } else {
                    ...
                    foreach ($existing_databases as $dbkey => $existing_database) {
                        foreach ($databases as $key => $database) {
                            if ($existing_database['name'] == $database->name) {
                                 ...
                                 $odatabase->update(Toolbox::addslashes_deep($dbinput), [], false);
                                 unset(
                   ...
                   //cleanup associated databases
                    if (count($existing_databases)) {
                        foreach ($existing_databases as $dbkey => $existing_database) {
                            $odatabase->delete(['id' => $dbkey], false, false);
                        }
                    }
                }
               ...
               //create new databases
                foreach ($databases as $database) {
                    ...
                    $odatabase->add(Toolbox::addslashes_deep($dbinput), [], false);
                }
