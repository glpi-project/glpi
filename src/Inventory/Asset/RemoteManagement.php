    ...
    public function handle()
    {
        ...
        foreach ($value as $k => $val) {
            ...
            foreach ($db_mgmt as $keydb => $arraydb) {
                ...
                if ($compare == $arraydb) {
                    ...
                    $this->updateItem($mgmt, $input, \Log::HISTORY_UPDATE_SUBITEM);
                    unset($value[$k]);
                    unset($db_mgmt[$keydb]);
                    break;
                }
        ...
        if (!$this->main_asset || !$this->main_asset->isPartial()) {
            foreach ($db_mgmt as $idtmp => $data) {
                if ($data['is_dynamic']) {
                    $this->deleteItem($mgmt, ['id' => $idtmp], \Log::HISTORY_DELETE_SUBITEM);
                }
            }
        }
        ...
        foreach ($value as $val) {
            ...
            $val->is_dynamic = 1;
            $this->addItem($mgmt, (array)$val, \Log::HISTORY_ADD_SUBITEM);
        }
    }
