...
abstract class AbstractCommonITILFormDestination extends AbstractFormDestinationType
{
    ...
    final public function createDestinationItems(
        ...
        foreach ($this->getConfigurableFields() as $field) {
            $use_auto_configuration = $config[$field->getKey() . "_auto"] ?? true;

            $input = $field->applyConfiguratedValueToInputUsingAnswers(
                $use_auto_configuration ? $field->getAutoGeneratedValue($form) : ($config[$field->getKey()] ?? null),
                $input,
                $answers_set
            );
        }
    ...
    final public function formatAutoConfigInputName(string $field_key): string
    {
        return $this->formatConfigInputName($field_key . "_auto");
    }
}
