...
class ActorConditionHandler implements ConditionHandlerInterface
{
    ...
    {
        // Process actor values directly in this method
        $actor_values = [];
        $values = is_iterable($value) ? $value : [$value];

        foreach ($values as $actor) {
            if (is_string($actor) && preg_match('/^(.+)-(.+)$/', $actor, $matches)) {
                $type = $matches[1];
                $id = $matches[2];
                if (!isset($actor_values[$type])) {
                    $actor_values[$type] = [];
                }
                $actor_values[$type][] = $id;
            }
        }
        ...
        return $renderer->renderFromStringTemplate($template, [
            'name'           => $name,
