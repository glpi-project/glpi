            ...
            switch ($opt["datatype"]) {
                ...
                case "progressbar":
                    ...
                    if (preg_match("/([<>])(=?)[[:space:]]*(-?)[[:space:]]*([0-9]+(\.[0-9]+)?)/", $val, $regs)) {
                        ...
                        $regs[1] .= $regs[2];
                        $tocompute_values = $tocompute instanceof QueryExpression ? $tocompute->getValues() : [];
                ...
                $replace_placeholders = static function ($add_criteria) use (&$replace_placeholders, $placeholders) {
                    ...
                    foreach ($add_criteria as $key => $value) {
                        ...
                        if (is_array($value)) {
                            ...
                            $new_criteria[$new_key] = new QueryExpression($new_value, values: $value->getValues());
                        } elseif ($value !== null) {
                ...
                if (
                    ...
                    if ($data['itemtype'] == AllAssets::class) {
                        ...
                        $tmpquery = $SELECT . ", '{$DB->escape($ctype)}' AS TYPE,
                            ...
                            . $FROM
                            . $WHERE;
                        $tmpvalues = array_merge($tmpvalues, $FROM_VALUES, $main_where->getParams());
