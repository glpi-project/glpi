...
class DbConfiguration extends AbstractRequirement
{
    ...
    /**
     * Whether the `max_allowed_packet` value must be checked.
     *
     * @var bool
     */
    ...
    public function __construct(DBmysql $db, bool $check_max_allowed_packet = false)
    {
        ...
        $this->db = $db;
        $this->check_max_allowed_packet = $check_max_allowed_packet;
    }
    ...
    protected function check()
    {
        ...
        if ($this->check_max_allowed_packet) {
            $query .= ', @@GLOBAL.' . $this->db->quoteName('max_allowed_packet as max_allowed_packet');
        }
        ...
        if (
            ...
            && (int) $db_config['max_allowed_packet'] < self::MIN_MAX_ALLOWED_PACKET
        ) {
