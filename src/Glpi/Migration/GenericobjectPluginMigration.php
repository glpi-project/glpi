...
class GenericobjectPluginMigration extends AbstractPluginMigration
{
    ...
    private function importAssetsDefinitions(): bool
    {
        ...
        foreach ($this->types_iterator as $type_data) {
            ...
            // Compute outdated state before trying to import the definition
            $is_outdated = false;
            $existing_definition = new AssetDefinition();
            if ($existing_definition->getFromDBByCrit($reconciliation_criteria)) {
                $is_outdated = \strtotime($type_data['date_mod']) < \strtotime($existing_definition->fields['date_mod']);
            }
            ...
            $profilerights_iterator = $this->db->request([
                ...
                'WHERE' => [
                    ...
                    'name' => preg_replace("/^glpi_/", "", $item_table),
                ],
            ]);
            ...
            foreach ($item_table_fields as $item_table_field) {
                if (!$this->isACustomField($type_data['itemtype'], $item_table_field)) {
                    continue;
                }

                $custom_field_specs = $this->getCustomFieldSpecs($type_data['itemtype'], $item_table_field);
            ...
            if (!$is_outdated) {
                ...
                foreach ($item_table_fields as $item_table_field) {
                    if (in_array($item_table_field, ['groups_id', 'groups_id_tech'])) {
                        ...
                        $target_field = $this->getTargetField($type_data['itemtype'], $item_table_field);
                    }
    ...
    private function importDropdowns(): bool
    {
        ...
        foreach ($this->types_iterator as $type_data) {
            ...
            $type_table     = \getTableForItemType($type_classname);
            if ($this->db->tableExists($type_table)) {
                $this->progress_indicator->setProgressBarMessage(
        ...
        foreach ($this->types_iterator as $type_data) {
            ...
            do {
                ...
                foreach ($assets_iterator as $asset_data) {
                    ...
                    // Compute outdated state before trying to import the asset
                    $is_outdated = false;
                    $existing_asset = new ($asset_definition->getAssetClassName())();
                    if ($existing_asset->getFromDBByCrit($reconciliation_criteria)) {
                        $is_outdated = \strtotime($asset_data['date_mod']) < \strtotime($existing_asset->fields['date_mod']);
                    }
