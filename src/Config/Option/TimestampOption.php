...
class TimestampOption extends ConfigOption
{
    public function __construct(
        ...
        string $context = 'core',
        int $min = 0,
        int $max = DAY_TIMESTAMP,
        int|null $step = null,
        bool $addfirstminutes = false,
        array $toadd = [],
        bool $inhours = false,
        ...
        $type_options = [
            'min' => $min,
            'max' => $max,
            'step' => $step ?? $CFG_GLPI["time_step"] * MINUTE_TIMESTAMP,
            'addfirstminutes' => $addfirstminutes,
            'toadd' => $toadd,
            'inhours' => $inhours,
            'emptylabel' => $emptylabel,
            'display_emptychoice' => $display_emptychoice
        ];
        parent::__construct($scopes, $section, $name, $label, InputType::DROPDOWN_TIMESTAMP, $type_options, $context);
    }
    ...
    public function renderInput(ConfigScope $scope, array $scope_params = [], array $input_params = []): void
    {
        ...
        {{ fields.dropdownTimestampField(name, value, label, field_options) }}
TWIG;
        $name = $this->getName();
