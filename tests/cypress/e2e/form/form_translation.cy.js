...
describe('Edit form translations', () => {
    ...
    it('translation remain visible when a translation has already been provided and the default value is emptied', () => {
        addTranslations();

        // Go to the form editor
        cy.findByRole('tab', { name: 'Form' }).click();

        // Empty the form description
        cy.findAllByLabelText('Form description')
            .awaitTinyMCE()
            .clear();
        ...
        // Go to the form translations page
        cy.findByRole('tab', { name: 'Form translations 1' }).click();

        // Open modal
        cy.findByRole('button', { name: 'Edit translation' }).click();
        cy.findByRole('dialog').should('have.attr', 'data-cy-shown', 'true');

        // Check that the translation for form description is visible and marked as "to review"
        cy.get('@formTranslationsTable').findAllByRole('row').as('formTranslationsRows');
        cy.get('@formTranslationsRows').eq(3).within(() => {
            cy.findByRole('cell', { name: 'Translation name' }).contains('Form description');
            cy.findByRole('cell', { name: 'Default value' }).should('contain.text', 'No default value');
            cy.findByRole('cell', { name: 'Translated value' })
                .findByLabelText('Enter translation')
                .parent().parent()
                .within(() => {
        ...
        // Clear the translation value
        cy.get('@formTranslationsRows').eq(3).within(() => {
            cy.findByLabelText('Enter translation').awaitTinyMCE().clear();
        });
        ...
        // Check that the translation for form description is no longer visible
        cy.get('@formTranslationsTable').findAllByRole('row').as('formTranslationsRows');
        cy.get('@formTranslationsRows').eq(-1).within(() => {
            cy.findByRole('cell', { name: 'Translation name' }).contains('Form description').should('not.exist');
        });
    });
});
