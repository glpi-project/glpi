...
function addTranslations() {
    cy.get('@form_id').then((form_id) => {
        cy.createWithAPI('Glpi\\Form\\Translation\\FormTranslation', {
            'forms_forms_id': form_id,
            'language': 'fr_FR',
        }).as('formTranslation_id');
    });
...
describe('Edit form translations', () => {
    beforeEach(() => {
        ...
        cy.get('@form_id').then((form_id) => {
            const tab = 'Glpi\\Form\\Translation\\FormTranslation$1';
    ...
    it('can add a new language translation', () => {
        ...
        cy.get('@formTranslations').find('tbody tr').eq(0).find('td').eq(1).findByRole('generic', { name: '0%' }).should('exist');
    ...
    it('check form translation stats', () => {
        ...
        cy.get('@form_id').then((form_id) => {
            cy.visit(`/front/form/form.form.php?id=${form_id}&forcetab=Glpi\\Form\\Translation\\FormTranslation$1`);
        });

        // Check stats
        cy.get('@formTranslations').find('tbody tr').eq(0).find('td').eq(1).findByRole('generic', { name: '50%' }).should('exist');
        cy.get('@formTranslations').find('tbody tr').eq(0).find('td').eq(2).invoke('text').then((text) => {
