...
describe ('Form editor', () => {
    ...
    it('can create and verify a horizontal block with questions and a comment', () => {
        ...
        cy.findByRole('region', {'name': 'Horizontal blocks layout'}).within(() => {
            ...
            cy.findAllByRole('region', {'name': 'Form horizontal block placeholder'}).eq(0).click();
            cy.findAllByRole('region', {'name': 'Form horizontal block placeholder'}).eq(0)
            ...
            cy.findByRole('region', {'name': 'Form horizontal block placeholder'}).click();
            cy.findByRole('region', {'name': 'Form horizontal block placeholder'})
    ...
    it('should not allow adding more than 4 placeholders in a horizontal block', () => {
        cy.findByRole('button', {'name': 'Add a horizontal layout'}).click();
        cy.findByRole('region', {'name': 'Horizontal blocks layout'}).within(() => {
            cy.findAllByRole('region', {'name': 'Form horizontal block placeholder'}).should('have.length', 2);
            for (let index = 3; index < 5; index++) {
                cy.findByRole('button', {'name': 'Add a new slot'}).click();
                cy.findAllByRole('region', {'name': 'Form horizontal block placeholder'}).should('have.length', index);
            }
