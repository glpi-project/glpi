...
describe('Service catalog page', () => {
    ...
    it('can navigate through the expanded service catalog using the breadcrumbs', () => {
        ...
        cy.findByRole('main').findByRole('navigation', { 'name': 'Service catalog categories' }).within(() => {
            ...
            cy.contains('a', `Root Category ${time}`).should('not.exist');
            cy.contains('a', `Child Category 1 ${time}`).should('not.exist');
            cy.contains('a', `Child Category 2 ${time}`).should('not.exist');
        });
