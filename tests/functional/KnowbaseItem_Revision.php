...
class KnowbaseItem_Revision extends DbTestCase
{
    ...
    public function testGetTabNameForItemLogged()
    {
        ...
        $_SESSION['glpishow_count_on_tabs'] = 1;
        $name = $kb_rev->getTabNameForItem($kb1);
        $this->string($name)->isIdenticalTo("<span><i class='ti ti-history me-2'></i>Revision</span> <span class='badge bg-secondary text-secondary-fg'>1</span>");
        ...
        $name = $kb_rev->getTabNameForItem($kb1);
        $this->string($name)->isIdenticalTo("<span><i class='ti ti-history me-2'></i>Revisions</span> <span class='badge bg-secondary text-secondary-fg'>2</span>");
