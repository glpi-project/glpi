...
class SavedSearch extends DbTestCase
{
    ...
    public function testAddVisibilityRestrict()
    {
       //first, as a super-admin
        ...
        $this->setEntity('_test_root_entity', true);
        $this->string(\SavedSearch::addVisibilityRestrict())
            ->isIdenticalTo("((`glpi_savedsearches`.`is_private` = '1' AND `glpi_savedsearches`.`users_id` = '5') OR (`glpi_savedsearches`.`is_private` = '0')) AND ((`glpi_savedsearches`.`entities_id` IN ('2', '3', '4') OR (`glpi_savedsearches`.`is_recursive` = '1' AND `glpi_savedsearches`.`entities_id` IN ('0'))))");
    }
