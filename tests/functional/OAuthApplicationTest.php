...
class OAuthApplicationTest extends DbTestCase
{
    ...
    public function testAddRequiresTenantIdForAzure(): void
    {
        // An empty tenant and an omitted tenant key (as an API payload would
        // send) must both be rejected, since Azure cannot be used without it.
        $tenant_inputs = [
            'empty tenant'   => ['tenant_id' => ''],
            'omitted tenant' => [],
        ];

        foreach ($tenant_inputs as $case => $tenant_input) {
            $app    = new OAuthApplication();
            $result = $app->prepareInputForAdd($tenant_input + [
                'name'          => 'Bad Azure app',
                'provider'      => 'azure',
                'client_id'     => 'cid',
                'client_secret' => 'secret',
            ...
            $this->assertFalse($result, 'Expected the Azure application to be rejected (' . $case . ')');
            $this->hasSessionMessages(ERROR, ['Tenant ID is required']);
        }
    }
