...
class HasProcessCapacity extends DbTestCase
{
    use CapacityUsageTestTrait;

    protected function getTargetCapacity(): string
    {
        return \Glpi\Asset\Capacity\HasProcessCapacity::class;
    }
    ...
    public function testCapacityActivation(): void
    {
        ...
        foreach ($has_capacity_mapping as $classname => $has_capacity) {
            ...
            $this->login();
            //no item in db, no tab for Item_Process
            $this->array($item->defineAllTabs())->notHasKey('Item_Process$1');
    ...
    public function testCapacityDeactivation(): void
    {
        ...
        $process_item_1 = $this->createItem(
            \Item_Process::class,
            [
        ...
        $process_item_2 = $this->createItem(
            \Item_Process::class,
            [
        ...
        $this->object(\Item_Process::getById($process_item_1->getID()))->isInstanceOf(\Item_Process::class);
        $this->integer(countElementsInTable(Log::getTable(), $item_1_logs_criteria))->isEqualTo(2); //create + add process
        $this->object(\Item_Process::getById($process_item_2->getID()))->isInstanceOf(\Item_Process::class);
        $this->integer(countElementsInTable(Log::getTable(), $item_2_logs_criteria))->isEqualTo(2); //create + add process
        ...
        $this->boolean(\Item_Process::getById($process_item_1->getID()))->isFalse();
    ...
    {
        yield [
            'target_classname' => \Item_Process::class
        ];
    }
    ...
    public function provideGetCapacityUsageDescription(): iterable
    {
        yield [
            'target_classname' => \Item_Process::class,
            'expected' => '%d process attached to %d assets'
        ];
    }
}
