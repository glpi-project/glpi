...
class HasSocketCapacity extends DbTestCase
{
    ...
    public function testCapacityActivation(): void
    {
        ...
        foreach ($has_capacity_mapping as $classname => $has_capacity) {
            ...
            if ($has_capacity) {
                $this->array($CFG_GLPI['socket_types'])->contains($classname);
            } else {
                $this->array($CFG_GLPI['socket_types'])->notContains($classname);
            }
            ...
            if ($has_capacity) {
                $this->array($item->defineAllTabs())->hasKey('Glpi\Socket$1');
            } else {
                $this->array($item->defineAllTabs())->notHasKey('Glpi\Socket$1');
            }
    ...
    public function testCapacityDeactivation(): void
    {
        ...
        $this->variable($cable_1->fields['itemtype_endpoint_a'])->isNull();
        $this->integer($cable_1->fields['items_id_endpoint_a'])->isEqualTo(0);
    }
