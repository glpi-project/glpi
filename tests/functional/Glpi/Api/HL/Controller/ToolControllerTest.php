...
class ToolControllerTest extends \HLAPITestCase
{
    ...
    {
        $this->login();
        $this->api->call(new Request('GET', '/Tool'), function ($call) {
            ...
            $call->response
                ...
                ->jsonContent(function ($content) {
                    ...
                    foreach ($content as $asset) {
                        ...
                        $this->assertEquals('/Tool/' . $asset['itemtype'], $asset['href']);
                    }
                });
        });
    }
    ...
    public function testAutoSearch()
    {
        ...
        $this->api->call(new Request('GET', '/Tool'), function ($call) use ($dataset) {
            /** @var \HLAPICallAsserter $call */
            $call->response
                ->isOK()
                ->jsonContent(function ($content) use ($dataset) {
                    ...
                    foreach ($content as $type) {
                        $this->api->autoTestSearch('/Tool/' . $type['itemtype'], $dataset);
                    }
                });
        });
    }
        ...
        $this->api->call(new Request('GET', '/Tool'), function ($call) {
            ...
            $call->response
                ...
                ->jsonContent(function ($content) {
                    $this->assertGreaterThanOrEqual(1, count($content));
                    foreach ($content as $type) {
                        $this->api->autoTestCRUD('/Tool/' . $type['itemtype']);
                    }
                });
        });
    }
