...
class RuleTicket extends RuleCommonITILObject
{
    ...
    public function testStatusAssignNewFromRule()
    {
       ...
       // Check ticket that trigger rule on update
        ...
        $this->integer(countElementsInTable(
            \Ticket_User::getTable(),
            ['tickets_id' => $tickets_id, 'type' => \CommonITILActor::ASSIGN, "users_id" => getItemByTypeName('User', TU_USER, true)]
        ...
        $this->integer(countElementsInTable(
            \Ticket_User::getTable(),
            ['tickets_id' => $tickets_id, 'type' => \CommonITILActor::ASSIGN, "users_id" =>getItemByTypeName('User', 'tech', true)]
        ))->isEqualTo(1); // Assigned only to tech (rule action assign)
    }
