...
test('tabs are refreshed on update', async ({ page, session, glpi_api }) => {
    ...
    await form_page.goToTab("Historical 4");
    await expect(form_page.history_rows).toHaveCount(5); // 4 entries + header
    ...
    await form_page.goToTab("Historical 4"); // Note that tab name isn't udpated so it still show 3 entries
    await expect(form_page.history_rows).toHaveCount(6); // 5 entries + header
});
