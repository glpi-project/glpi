...
if(isset($_POST["item_type"][0])&&$_POST["item_type"][0] != '0')
{
	...
	foreach($_POST["item_type"] as $key => $val){
		$query = "select ".$item_db_name[$val].".name as itemname, ".$item_db_name[$val].".deleted as itemdeleted, ".$item_db_name[$val].".*, glpi_contracts.*, glpi_infocoms.* from ".$item_db_name[$val]." ";
		$query.= "LEFT JOIN glpi_contract_device ON glpi_contract_device.device_type='$val' AND ".$item_db_name[$val].".ID =  glpi_contract_device.FK_device ";
		$query.= "LEFT JOIN glpi_contracts ON glpi_contract_device.FK_contract=glpi_contracts.ID AND glpi_contract_device.FK_contract IS NOT NULL ";
...
else
{
	$query=array();
		...
		{
		$query[$key] = "select ".$item_db_name[$key].".name as itemname, ".$item_db_name[$key].".deleted as itemdeleted, ".$item_db_name[$key].".*, glpi_contracts.*, glpi_infocoms.* from ".$item_db_name[$key]." ";
		$query[$key].= "LEFT JOIN glpi_contract_device ON glpi_contract_device.device_type='$key' AND ".$item_db_name[$key].".ID =  glpi_contract_device.FK_device ";
		$query[$key].= "LEFT JOIN glpi_contracts ON glpi_contract_device.FK_contract=glpi_contracts.ID AND glpi_contract_device.FK_contract IS NOT NULL ";
		...
		$query[$key].= " order by ".$item_db_name[$key].".name asc";
//		echo $query[$key];
