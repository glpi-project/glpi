	...
	if ($ID){
		...
		if ($DB->numrows($result)==1){
			...
			if ($link==2){
				...
				if ($data["title"]>0)
					$user["comments"].=$LANG['common'][81].": ".getDropdownName("glpi_dropdown_user_titles",$data["title"])."<br>";
				if ($data["type"]>0)
					$user["comments"].=$LANG['common'][17].": ".getDropdownName("glpi_dropdown_user_types",$data["type"])."<br>";
			} else {
				$user=$username;
			}
		}
