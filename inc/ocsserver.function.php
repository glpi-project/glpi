               ...
               if ($data) {
                  ...
                  if ($res_id = $DB->query($sql_id)) {
                     if ($DB->numrows($res_id)>0) {
                        ...
                        //New ocsid
                        $changes[2]=$data["ID"];
                        historyLog ($DB->result($res_id,0,"computers_id"),COMPUTER_TYPE,$changes,0,
   ...
   if (!$ocs_exists || $numrows == 0) {
      ...
      if ($idlink = ocsLink($ocsid, $ocsservers_id, $computers_id)) {
         ...
         if ($cfg_ocs["import_monitor"] == 1) { // Only reset monitor as global in unit management
            ocsResetMonitors($computers_id);    // try to link monitor with existing
         }
   ...
   if ($DB->numrows($result) == 1) {
      ...
      if ($DBocs->numrows($result_ocs) == 1) {
         ...
         if ($mixed_checksum) {
            ...
            ocsUpdateAdministrativeInfo($line['computers_id'], $line['ocsid'], $ocsservers_id,
                                        $cfg_ocs, $computer_updates, $comp->fields['entities_id'],
                                        $dohistory);
            if ($mixed_checksum & pow(2, HARDWARE_FL)) {
               $loghistory = ocsUpdateHardware($line['computers_id'], $line['ocsid'], $ocsservers_id,
                                               $cfg_ocs, $computer_updates, $dohistory);
            }
            if ($mixed_checksum & pow(2, BIOS_FL)) {
               ocsUpdateBios($line['computers_id'], $line['ocsid'], $ocsservers_id, $cfg_ocs,
                             $computer_updates, $dohistory);
            }
            // Get import devices
            $import_device = importOcsArrayFromDB($line["import_device"]);
            if ($mixed_checksum & pow(2, MEMORIES_FL)) {
               ocsUpdateDevices(RAM_DEVICE, $line['computers_id'], $line['ocsid'], $ocsservers_id,
                                $cfg_ocs, $import_device, '', $dohistory);
            }
            if ($mixed_checksum & pow(2, STORAGES_FL)) {
               ocsUpdateDevices(HDD_DEVICE, $line['computers_id'], $line['ocsid'], $ocsservers_id,
                                $cfg_ocs, $import_device, '', $dohistory);
               ocsUpdateDevices(DRIVE_DEVICE, $line['computers_id'], $line['ocsid'], $ocsservers_id,
                                $cfg_ocs, $import_device, '', $dohistory);
            }
            if ($mixed_checksum & pow(2, HARDWARE_FL)) {
               ocsUpdateDevices(PROCESSOR_DEVICE, $line['computers_id'], $line['ocsid'],
                                $ocsservers_id, $cfg_ocs, $import_device, '', $dohistory);
            }
            if ($mixed_checksum & pow(2, VIDEOS_FL)) {
               ocsUpdateDevices(GFX_DEVICE, $line['computers_id'], $line['ocsid'], $ocsservers_id,
                                $cfg_ocs, $import_device, '', $dohistory);
            }
            if ($mixed_checksum & pow(2, SOUNDS_FL)) {
               ocsUpdateDevices(SND_DEVICE, $line['computers_id'], $line['ocsid'], $ocsservers_id,
                                $cfg_ocs, $import_device, '', $dohistory);
            }
            if ($mixed_checksum & pow(2, NETWORKS_FL)) {
               $import_ip = importOcsArrayFromDB($line["import_ip"]);
               ocsUpdateDevices(NETWORK_DEVICE, $line['computers_id'], $line['ocsid'],
                                $ocsservers_id, $cfg_ocs, $import_device, $import_ip, $dohistory);
            }
            if ($mixed_checksum & pow(2, MODEMS_FL) || $mixed_checksum & pow(2, PORTS_FL)) {
               ocsUpdateDevices(PCI_DEVICE, $line['computers_id'], $line['ocsid'], $ocsservers_id,
                                $cfg_ocs, $import_device, '', $dohistory);
            }
            if ($mixed_checksum & pow(2, MONITORS_FL)) {
               // Get import monitors
               $import_monitor = importOcsArrayFromDB($line["import_monitor"]);
               ocsUpdatePeripherals(MONITOR_TYPE, $comp->fields["entities_id"], $line['computers_id'],
                                    $line['ocsid'], $ocsservers_id, $cfg_ocs, $import_monitor,
                                    $dohistory);
            }
            if ($mixed_checksum & pow(2, PRINTERS_FL)) {
               // Get import printers
               $import_printer = importOcsArrayFromDB($line["import_printer"]);
               ocsUpdatePeripherals(PRINTER_TYPE, $comp->fields["entities_id"], $line['computers_id'],
                                    $line['ocsid'], $ocsservers_id, $cfg_ocs, $import_printer,
                                    $dohistory);
            }
            if ($mixed_checksum & pow(2, INPUTS_FL)) {
               // Get import peripheral
               $import_peripheral = importOcsArrayFromDB($line["import_peripheral"]);
               ocsUpdatePeripherals(PERIPHERAL_TYPE, $comp->fields["entities_id"],
                                    $line['computers_id'], $line['ocsid'], $ocsservers_id, $cfg_ocs,
                                    $import_peripheral, $dohistory);
            }
            if ($mixed_checksum & pow(2, SOFTWARES_FL)) {
               // Get import software
               $import_software = importOcsArrayFromDB($line["import_software"]);
               ocsUpdateSoftware($line['computers_id'], $comp->fields["entities_id"], $line['ocsid'],
                                 $ocsservers_id, $cfg_ocs, $import_software,
                                 (!$loghistory["history"]?0:$dohistory));
            }
            if ($mixed_checksum & pow(2, DRIVES_FL)) {
               // Get import drives
               $import_disk = importOcsArrayFromDB($line["import_disk"]);
               ocsUpdateDisk($line['computers_id'], $line['ocsid'], $ocsservers_id, $cfg_ocs,
                             $import_disk);
            }
 ...
 *
 **/
function ocsUpdateHardware($computers_id, $ocsid, $ocsservers_id, $cfg_ocs, $computer_updates,
                           $dohistory = 2) {
 ...
 *
 **/
function ocsUpdateBios($computers_id, $ocsid, $ocsservers_id, $cfg_ocs, $computer_updates,
                       $dohistory = 2) {
   global $DBocs;
 ...
 *
 **/
function ocsUpdateDevices($devicetype, $computers_id, $ocsid, $ocsservers_id, $cfg_ocs,
                     ...
                     if (!in_array(RAM_DEVICE . OCS_FIELD_SEPARATOR . $ram["designation"],
                        ...
                        if ($ram_id) {
                           $devID = compdevice_add($computers_id, RAM_DEVICE, $ram_id,
                                                   $line2["CAPACITY"], $dohistory);
                           addToOcsArray($computers_id,
                                         array($devID => RAM_DEVICE .
                                                         OCS_FIELD_SEPARATOR . $ram["designation"]),
                                         "import_device");
                        }
                     } else {
                        $id = array_search(RAM_DEVICE . OCS_FIELD_SEPARATOR . $ram["designation"],
                                           $import_device);
            ...
            if ($DBocs->numrows($result2) > 0) {
               while ($line2 = $DBocs->fetch_array($result2)) {
                  ...
                  if (!empty ($line2["DISKSIZE"]) && preg_match("/disk/i", $line2["TYPE"])) {
                     ...
                     if (!in_array(HDD_DEVICE . OCS_FIELD_SEPARATOR . $dd["designation"],
                                   $import_device)) {
                        $dd["specif_default"] = $line2["DISKSIZE"];
                        $dd_id = ocsAddDevice(HDD_DEVICE, $dd);
                        if ($dd_id) {
                           $devID = compdevice_add($computers_id, HDD_DEVICE, $dd_id,
                                                   $line2["DISKSIZE"], $dohistory);
                           addToOcsArray($computers_id,
                                         array($devID => HDD_DEVICE .
                                                         OCS_FIELD_SEPARATOR . $dd["designation"]),
                                         "import_device");
                        }
                     } else {
                        $id = array_search(HDD_DEVICE . OCS_FIELD_SEPARATOR . $dd["designation"],
                                           $import_device);
            ...
            if ($DBocs->numrows($result2) > 0) {
               while ($line2 = $DBocs->fetch_array($result2)) {
                  ...
                  if (empty ($line2["DISKSIZE"]) || !preg_match("/disk/i", $line2["TYPE"])) {
                     ...
                     if (!in_array(DRIVE_DEVICE . OCS_FIELD_SEPARATOR . $stor["designation"],
                                   $import_device)) {
                        $stor["specif_default"] = $line2["DISKSIZE"];
                        $stor_id = ocsAddDevice(DRIVE_DEVICE, $stor);
                        if ($stor_id) {
                           $devID = compdevice_add($computers_id, DRIVE_DEVICE, $stor_id, "",
                                                   $dohistory);
                           addToOcsArray($computers_id,
                                         array($devID => DRIVE_DEVICE . OCS_FIELD_SEPARATOR .
                                                         $stor["designation"]),
                                         "import_device");
                        }
                     } else {
                        $id = array_search(DRIVE_DEVICE . OCS_FIELD_SEPARATOR . $stor["designation"],
                                           $import_device);
                        unset ($import_device[$id]);
                     }
                  }
         ...
         if ($cfg_ocs["import_device_modem"]) {
            ...
            if ($DBocs->numrows($result2) > 0) {
               while ($line2 = $DBocs->fetch_array($result2)) {
                  ...
                  if (!in_array(PCI_DEVICE . OCS_FIELD_SEPARATOR . $mdm["designation"],
                                $import_device)) {
                     ...
                     if ($mdm_id) {
                        ...
                        addToOcsArray($computers_id,
                                      array($devID => PCI_DEVICE . OCS_FIELD_SEPARATOR .
                                                      $mdm["designation"]),
                                      "import_device");
                     }
                  } else {
                     $id = array_search(PCI_DEVICE . OCS_FIELD_SEPARATOR . $mdm["designation"],
                                        $import_device);
                     unset ($import_device[$id]);
                  }
               }
            ...
            if ($DBocs->numrows($result2) > 0) {
               while ($line2 = $DBocs->fetch_array($result2)) {
                  ...
                  if (!empty ($port["designation"])) {
                     if (!in_array(PCI_DEVICE . OCS_FIELD_SEPARATOR . $port["designation"],
                                   $import_device)) {
                        ...
                        if ($port_id) {
                           $devID = compdevice_add($computers_id, PCI_DEVICE, $port_id, "",
                                                   $dohistory);
                           addToOcsArray($computers_id,
                                         array($devID => PCI_DEVICE . OCS_FIELD_SEPARATOR .
                                                         $port["designation"]),
                        ...
                        $id = array_search(PCI_DEVICE . OCS_FIELD_SEPARATOR . $port["designation"],
                                           $import_device);
                        unset ($import_device[$id]);
                     }
                  }
         ...
         if ($cfg_ocs["import_device_processor"]) {
            ...
            if ($DBocs->numrows($result) == 1) {
               ...
               for ($i = 0; $i < $line["PROCESSORN"]; $i++) {
                  ...
                  if (!in_array(PROCESSOR_DEVICE . OCS_FIELD_SEPARATOR . $processor["designation"],
                                $import_device)) {
                     $proc_id = ocsAddDevice(PROCESSOR_DEVICE, $processor);
                     if ($proc_id) {
                        $devID = compdevice_add($computers_id, PROCESSOR_DEVICE, $proc_id,
                                                $line["PROCESSORS"], $dohistory);
                        addToOcsArray($computers_id,
                                      array($devID => PROCESSOR_DEVICE . OCS_FIELD_SEPARATOR .
                                                      $processor["designation"]),
                                      "import_device");
                     }
                  } else {
                     $id = array_search(PROCESSOR_DEVICE . OCS_FIELD_SEPARATOR .
            ...
            if ($DBocs->numrows($result2) > 0) {
               while ($line2 = $DBocs->fetch_array($result2)) {
                  ...
                  if ($cfg_ocs["import_device_iface"]) {
                     ...
                     if (!in_array(NETWORK_DEVICE . OCS_FIELD_SEPARATOR . $network["designation"],
                                   $import_device)) {
                        ...
                        if ($net_id) {
                           $devID = compdevice_add($computers_id, NETWORK_DEVICE, $net_id,
                                                   $line2["MACADDR"], $dohistory);
                           addToOcsArray($computers_id,
                                         array($devID => NETWORK_DEVICE . OCS_FIELD_SEPARATOR .
                                                         $network["designation"]),
                                         "import_device");
                        }
                     } else {
                        $id = array_search(NETWORK_DEVICE . OCS_FIELD_SEPARATOR .
                  ...
                  if (!empty ($line2["IPADDRESS"]) && $cfg_ocs["import_ip"]) {
                     ...
                     if ($count_ip == 1) {
                        ...
                        if ($DB->numrows($result) > 0) {
                           while ($data = $DB->fetch_array($result)) {
                              ...
                              addToOcsArray($computers_id,
                                            array($data["id"] => $data["ip"] . OCS_FIELD_SEPARATOR .
                                                                 $line2["MACADDR"]),
                                            "import_ip");
                     ...
                     for ($j = 0; $j < count($ocs_ips); $j++) {
                           ...
                           foreach($import_ip as $ID => $ip) {
                              if ($ID > 0) {
                                 ...
                                 if (isset($tmp[1]) && $tmp[1] == $line2["MACADDR"]) {
                                    ...
                                    addToOcsArray($computers_id,
                                                  array($ID => $ocs_ips[$j] . OCS_FIELD_SEPARATOR .
                                                               $line2["MACADDR"]),
                                                  "import_ip");
                                    $import_ip[$ID]= $ocs_ips[$j] . OCS_FIELD_SEPARATOR .
                                                     $line2["MACADDR"];
                                    $id_ip = $ID;
                                    break;
                                 }
                        ...
                        if ($id_ip>0) {
                           ...
                           //ADD to array
                           addToOcsArray($computers_id,
                                         array($newID => $ocs_ips[$j] . OCS_FIELD_SEPARATOR .
                                                         $line2["MACADDR"]),
                                         "import_ip");
                           $count_ip++;
                        }
         ...
         if ($cfg_ocs["import_device_gfxcard"]) {
            ...
            if ($DBocs->numrows($result2) > 0) {
               while ($line2 = $DBocs->fetch_array($result2)) {
                  ...
                  if (!in_array(GFX_DEVICE . OCS_FIELD_SEPARATOR . $video["designation"],
                                $import_device)) {
                     ...
                     if ($video_id) {
                        $devID = compdevice_add($computers_id, GFX_DEVICE, $video_id,
                                                $video["specif_default"], $dohistory);
                        addToOcsArray($computers_id,
                                      array($devID => GFX_DEVICE . OCS_FIELD_SEPARATOR .
                                                      $video["designation"]),
                                      "import_device");
                     }
                  } else {
                     $id = array_search(GFX_DEVICE . OCS_FIELD_SEPARATOR . $video["designation"],
                                        $import_device);
         ...
         if ($cfg_ocs["import_device_sound"]) {
            ...
            if ($DBocs->numrows($result2) > 0) {
               while ($line2 = $DBocs->fetch_array($result2)) {
                  ...
                  if (!in_array(SND_DEVICE . OCS_FIELD_SEPARATOR . $snd["designation"],
                                $import_device)) {
                     if (!empty ($line2["DESCRIPTION"])) {
                     ...
                     if ($snd_id) {
                        ...
                        addToOcsArray($computers_id,
                                      array($devID => SND_DEVICE . OCS_FIELD_SEPARATOR .
                                                      $snd["designation"]),
                                      "import_device");
                     }
                  } else {
                     $id = array_search(SND_DEVICE . OCS_FIELD_SEPARATOR . $snd["designation"],
                                        $import_device);
                     unset ($import_device[$id]);
                  }
               }
 ...
 *
 **/
function ocsUpdatePeripherals($itemtype, $entity, $computers_id, $ocsid, $ocsservers_id, $cfg_ocs,
         ...
         if ($cfg_ocs["import_monitor"]) {
            ...
            if (!in_array(OCS_IMPORT_TAG_070, $import_periph)) {
               foreach ($import_periph as $key => $val) {
                  ...
                  addToOcsArray($computers_id, array ($key => $monitor_tag), "import_monitor");
               ...
               foreach ($lines as $line) {
                  ...
                  if ($id === false) {
                     ...
                     if ($id_monitor) {
                        ...
                        addToOcsArray($computers_id, array ($connID => $checkMonitor),
                                      "import_monitor");
                        $count_monitor++;
 ...
 *
 **/
function ocsUpdateAdministrativeInfo($computers_id, $ocsid, $ocsservers_id, $cfg_ocs,
...
function ocsUpdateRegistry($computers_id, $ocsid, $ocsservers_id, $cfg_ocs) {
   ...
   if ($cfg_ocs["import_registry"]) {
      ...
      $query = "SELECT `registry`.`NAME` AS name, `registry`.`REGVALUE` AS regvalue,
 ...
 *
 **/
function ocsUpdateSoftware($computers_id, $entity, $ocsid, $ocsservers_id, $cfg_ocs,
         ...
         if ($line['TOTAL']>0) {
            ...
            if (isset($disk['name']) && !empty($disk["name"])) {
               ...
               if (!in_array($disk["name"], $import_disk)) {
                  ...
                  $id_disk = $d->add($disk);
                  if ($id_disk) {
                     addToOcsArray($computers_id, array ($id_disk => $disk["name"]), "import_disk");
                  }
               } else {
   ...
   if (count($import_ip) > 1) {
      foreach($import_ip as $importip_ID => $value) {
         if ($importip_ID > 0) {
            ...
            addToOcsArray($computers_id,array($importip_ID=>$new_ip . OCS_FIELD_SEPARATOR . $new_mac),
                          "import_ip");
