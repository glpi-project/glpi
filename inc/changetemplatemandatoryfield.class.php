...
/// since version 9.3
class ChangeTemplateMandatoryField extends CommonDBChild {
   ...
   **/
   static function showForChangeTemplate(ChangeTemplate $ct, $withtemplate = 0) {
      ...
      $ctm               = new self();
      $used              = $ctm->getMandatoryFields($ID);
      $fields            = $ct->getAllowedFieldsNames(true);
      $fields            = array_diff_key($fields, self::getExcludedFields());

      $rand  = mt_rand();

      $query = "SELECT `glpi_changetemplatemandatoryfields`.*
                FROM `glpi_changetemplatemandatoryfields`
      ...
      if ($result = $DB->query($query)) {
         ...
         if ($canedit) {
            ...
            foreach ($select_fields as $key => $val) {
               $select_fields[$key] = sprintf(__('%1$s'), $val);
            }
