   ...
   static function reportForNetworkInformations(
      ...
      $criteria = [
         'SELECT'       => array_merge([
            ...
            new QueryExpression('GROUP_CONCAT(' . $DB->quoteName('ADDR_1.name') . ' SEPARATOR ' . $DB->quoteValue(',') . ') AS ' . $DB->quoteName('ip_1')),
            'PORT_2.itemtype AS itemtype_2',
            'PORT_2.items_id AS items_id_2',
            'PORT_2.id AS id_2',
            'PORT_2.name AS port_2',
            'PORT_2.mac AS mac_2',
            new QueryExpression('GROUP_CONCAT(' . $DB->quoteName('ADDR_2.name') .' SEPARATOR ' . $DB->quoteValue(',') . ') AS ' . $DB->quoteName('ip_2'))
         ], $select),
         'FROM'         => $from,
