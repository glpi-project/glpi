...
class ReminderTranslation extends CommonDBChild {
   ...
   static function showTranslations(Reminder $item) {
      ...
      if (count($found) > 0) {
         ...
         foreach ($found as $data) {
            ...
            echo "</td><td>";
            if ($canedit) {
               echo "<a href=\"" . $CFG_GLPI["root_doc"] . "/front/remindertranslation.form.php?id=".$data["id"] . "\">{$data['name']}</a>";
            } else {
               echo  $data["name"];
            }
   ...
   **/
   static function canBeTranslated(CommonDBVisible $item) {
