...
/// since version 9.3
class ChangeTemplateHiddenField extends CommonDBChild {
   ...
   **/
   function getRawName() {
      ...
      $fields = $ct->getAllowedFieldsNames(true);
   ...
   function getTabNameForItem(CommonGLPI $item, $withtemplate = 0) {
      ...
      if (($item->getType() == 'ChangeTemplate')
          && Session::haveRight("changetemplate", READ)) {
         $nb = 0;
         if ($_SESSION['glpishow_count_on_tabs']) {
            $nb = countElementsInTable($this->getTable(),
                                       ['changetemplates_id' => $item->getID()]);
         }
   ...
   static function displayTabContentForItem(CommonGLPI $item, $tabnum = 1, $withtemplate = 0) {
      ...
      self::showForChangeTemplate($item, $withtemplate);
      return true;
   }
   ...
   function post_purgeItem() {
      ...
      $change      = new Change();
      $itemtype_id = $change->getSearchOptionIDByField('field', 'itemtype', 'glpi_changes');
      $items_id_id = $change->getSearchOptionIDByField('field', 'items_id', 'glpi_changes');

      // Try to delete itemtype -> delete items_id
      if ($this->fields['num'] == $itemtype_id) {
         $query = "SELECT `id`
                   FROM `".$this->getTable()."`
                   WHERE `".static::$items_id."` = '".$this->fields['changetemplates_id']."'
                         AND `num` = '$items_id_id'";
   ...
   **/
   function getHiddenFields($ID, $withtypeandcategory = false) {
      ...
      $ct             = new ChangeTemplate();
      $allowed_fields = $ct->getAllowedFields($withtypeandcategory);
      $fields         = [];
    ...
    * @since version 9.3
    *
    * @param $ct                       Change Template
    * @param $withtemplate    boolean  Template or basic item (default 0)
    *
    * @return Nothing (call to classes members)
   **/
   static function showForChangeTemplate(ChangeTemplate $ct, $withtemplate = 0) {
      global $DB;

      $ID = $ct->fields['id'];

      if (!$ct->getFromDB($ID) || !$ct->can($ID, READ)) {
         return false;
      }

      $ctm     = new self();
      $used    = $ctm->getHiddenFields($ID);

      $canedit = $ct->canEdit($ID);
      $fields  = $ct->getAllowedFieldsNames(false);
      $fields  = array_diff_key($fields, self::getExcludedFields());
      $rand    = mt_rand();

      $query = "SELECT `glpi_changetemplatehiddenfields`.*
                FROM `glpi_changetemplatehiddenfields`
                WHERE (`changetemplates_id` = '$ID')";
      ...
      if ($result = $DB->query($query)) {
         ...
         if ($canedit) {
            ...
            echo "<input type='hidden' name='changetemplates_id' value='$ID'>";
