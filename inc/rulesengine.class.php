...
class Rule extends CommonDBTM{
	...
	function showForm($target,$ID,$withtemplate=''){
			...
			if ($canedit)
			{
				if ($new){
					...
					echo "<tr><td class='tab_bg_2' align='center' colspan='4'>";
					echo $LANG["rulesengine"][47]."&nbsp;";
		
					echo "<img alt='' src='".$CFG_GLPI["root_doc"]."/pics/aide.png'\" ";
					echo " style='cursor:pointer;'  onClick=\"window.open('".$CFG_GLPI["root_doc"]."/front/popup.php?popup=test_rule&amp;rule_type=".$this->rule_type."&amp;rule_id=".$this->fields["ID"]."' ,'glpipopup', 'height=400, width=1000, top=100, left=100, scrollbars=yes' )\"";
					echo "</td></tr>";
	...
	* @param $canedit right to edit ?
	...
	function testRuleForm($target,$rule_id,$test_rule_output)
	{
		global $DB, $LANG,$RULES_CRITERIAS,$RULES_ACTIONS;
		if ($this->getFromDB($rule_id))
		{
			$this->getRuleWithCriteriasAndActions($rule_id,1,0);
			echo "<br><br><form name='testrule_form' id='testrule_form' method='post' action=\"$target\">\n";
			echo "<div class='center'>"; 
			echo "<table class='tab_cadre_fixe'>";
			echo "<tr><th colspan='2'>" . $LANG["rulesengine"][48] . "</th></tr>";
			
			$first = true;
			$display_criterias=array();
			...
			foreach ($this->criterias as $criteria)
			{
				...
				if (!in_array($criteria->fields["criteria"],$display_criterias))
				{
					if (!$first)
					{
						echo "<tr>";
						echo "<td class='tab_bg_2' align='center' colspan='2'>".($this->fields["match"]?$LANG["rulesengine"][74]:$LANG["rulesengine"][75])."</td>"; 
						echo "</tr>";
					}				
					...
					if (!isset($criteria_constants["type"]))
						$criteria_constants["type"]="text";
	
					echo "<td class='tab_bg_2' align='center'>".$LANG["rulesengine"][16]." : ".$criteria_constants["name"]."</td>";

					echo "<td class='tab_bg_2' align='center'>";
					
					//Display criteria : for some specific types, display a combobox instead of text field
					switch($criteria->fields["criteria"])
					{
						...
						case "assign":
						case "FK_entities":
							dropdownValue($criteria_constants["table"],$criteria->fields["criteria"],(isset($_POST[$criteria->fields["criteria"]])?$_POST[$criteria->fields["criteria"]]:'0'));
						break;	
						default:
							autocompletionTextField($criteria->fields["criteria"],$criteria_constants["table"],$criteria_constants["field"],(isset($_POST[$criteria->fields["criteria"]])?$_POST[$criteria->fields["criteria"]]:''));
						break;
						case "priority":
							dropdownPriority($criteria->fields["criteria"],(isset($_POST[$criteria->fields["criteria"]])?$_POST[$criteria->fields["criteria"]]:'0'));
						break;						
					}
					echo "</td>";
					echo "</tr>";
					$first=false;
					$display_criterias[]=$criteria->fields["criteria"];
				}
			}
			
			echo "<tr><td class='tab_bg_2' colspan='2' align='center'>";
			echo "<input type='submit' name='test_rule' value=\"" . $LANG["rulesengine"][47] . "\" class='submit'>";
			echo "<input type='hidden' name='rule_id' value=\"" . $rule_id . "\">";
			echo "<input type='hidden' name='rule_type' value=\"" . $this->rule_type . "\">";
			echo "</td></tr>";
			echo "</table></div></form>";
			
			if (is_array($test_rule_output))
			{
				echo "<br><br><form name='resulttestrule_form' id='resulttestrule_form' method='post' action=\"$target\">\n";
				echo "<div class='center'>"; 
				echo "<table class='tab_cadre_fixe'>";
				echo "<tr><th colspan='2'>" . $LANG["rulesengine"][49] . "</th></tr>";

				if (count($test_rule_output)>0 && in_array("_rule_process",$test_rule_output) && $test_rule_output["_rule_process"]==1)
				{
					unset($test_rule_output["_rule_process"]);
					foreach ($test_rule_output as $field => $result)
					{
						$display_oneline=true;
						switch ($field)
						{
							case "active":
								((!$result)?$display_result=$LANG["choice"][0]:$display_result=$LANG["choice"][1]);
							break;
							//Do not display entity ID but name instead
							case "FK_entities":
								$display_result=getDropdownName("glpi_entities",$result);
							break;	
							case "category":
								$display_result=getDropdownName("glpi_dropdown_tracking_category",$result);
							break;	
							
							//Right affectation rule
							case "_ldap_rules":
								$display_oneline=false;
								$display_result="";
								
								foreach($result as $rule_tabname => $rules)
								{
									switch ($rule_tabname)
									{
										case "rules_entities":
											$display_result.="<tr><th class='tab_bg_2' align='center' colspan='2'>".$LANG["rulesengine"][72]."</th></tr>";
											foreach($rules as $entity)
											{
												$display_result.="<tr>";
												$display_result.="<td class='tab_bg_2' align='center'>".$LANG["entity"][0]." </td>\n";
												$display_result.="<td class='tab_bg_2' align='center'>";									
												$display_result.=getDropdownName("glpi_entities",$entity[0]); 
												$display_result.="</td>";
												$display_result.="</tr>";
													
												$display_result.="<tr>";
												$display_result.="<td class='tab_bg_2' align='center'>".$LANG["profiles"][28]." </td>\n";
												$display_result.="<td class='tab_bg_2' align='center'>";									
												$display_result.=((!$entity[1])?$LANG["choice"][0]:$LANG["choice"][1]);
												$display_result.="</td>";
												$display_result.="</tr>";
											}
										break;
										case "rules_rights":
											$display_result.="<tr><th class='tab_bg_2' align='center' colspan='2'>".$LANG["rulesengine"][73]."</th></tr>";										
											foreach($rules as $id=>$profile)
											{
												$display_result.="<tr><td class='tab_bg_2' align='center'>".$LANG["entity"][0]."</td>\n";
												$display_result.="<td class='tab_bg_2' align='center'>";									
												$display_result.=getDropdownName("glpi_profiles",$profile); 
												$display_result.="</td></tr>";
											}
										break;
										case "rules_entities_rights":
											$display_result.="<tr><th class='tab_bg_2' align='center' colspan='2'>".$LANG["rulesengine"][71]."</th></tr>";
											...
											foreach($rules as $entity)
											{
													...
													$display_result.="<tr>";
													$display_result.="<td class='tab_bg_2' align='center'>".$LANG["Menu"][35]." </td>\n";
													$display_result.="<td class='tab_bg_2' align='center'>";									
													$display_result.=getDropdownName("glpi_profiles",$entity[1]); 
													$display_result.="</td>";
													$display_result.="</tr>";
													...
													$display_result.="<td class='tab_bg_2' align='center'>".$LANG["profiles"][28]." </td>\n";
													$display_result.="<td class='tab_bg_2' align='center'>";									
													$display_result.=((!$entity[2])?$LANG["choice"][0]:$LANG["choice"][1]);
													$display_result.="</td>";
													$display_result.="</tr>";
											}
									}
							...
							if ($display_oneline)
							{
								if (isset($RULES_ACTIONS[$this->rule_type][$field]["name"]))
									echo "<tr><td class='tab_bg_2' align='center'>".$RULES_ACTIONS[$this->rule_type][$field]["name"]." </td>";
								else	
									echo "<tr><td class='tab_bg_2' align='center'>".$field." </td>";
								echo "<td class='tab_bg_2' align='center'>".$display_result."</td></tr>";
							}
							else
								echo $display_result;
					...
					echo "<tr><td class='tab_bg_2' align='center'>".$LANG["rulesengine"][70]."</td></tr>";
