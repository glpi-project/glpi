...
/**
 * ChangeFollowup Class
**/
class ChangeFollowup  extends CommonITILFollowup {
   ...
   **/
      ...
      if (isset($input["add_reopen"])) {
         if ($input["content"] == '') {
            if (isset($input["_add"])) {
               ...
               Session::addMessageAfterRedirect(__('If you want to reopen the change, you must specify a reason'),
                                                false, ERROR);
            } else {
               // Refuse solution
   ...
   function post_addItem() {
      ...
      if (isset($this->input["_close"])
         ...
         $donotif = false; // Done for Change update (new status)
      }

      //manage reopening of Change
      $reopened = false;
      ...
      //change Change status only if imput change
      if (!$reopened
          && $this->input['_status'] != $this->input['_job']->fields['status']) {
         ...
         // don't notify on Change - update event
