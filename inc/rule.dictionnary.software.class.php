...
class DictionnarySoftwareCollection extends RuleCachedCollection {
   ...
   **/
   ...
   function warningBeforeReplayRulesOnExistingDB($target) {
      ...
      echo "<input type='submit' name='replay_rule' value=\"" . $LANG['buttons'][2] .
            "\" class='submit'><input type='hidden' name='replay_confirm' value='replay_confirm'";
      echo "</td></tr>";
      echo "</table>\n";
      echo "</div></form>\n";
      return true;
   ...
   function replayDictionnaryOnSoftwaresByID($IDs, $res_rule = array ()) {
      ...
      foreach ($IDs as $ID) {
         $res_soft = $DB->query("SELECT `gs`.`id`, `gs`.`name` AS name,
   ...
   function putOldSoftsInTrash($soft_ids) {
      ...
      if (count($soft_ids) > 0) {
         ...
         $res_countsoftinstall = $DB->query("SELECT `glpi_softwares`.`id`,
                                                    COUNT(`glpi_softwaresversions`.`softwares_id`) AS cpt
                                             FROM `glpi_softwares`
                                             LEFT JOIN `glpi_softwaresversions`
                                                   ON `glpi_softwaresversions`.`softwares_id`
                                                      = `glpi_softwares.id`
   ...
   function showCacheRuleDetail($fields) {
      ...
      echo "<td class='tab_bg_2'>" .
            ((isset ($fields["new_manufacturer"]) && $fields["new_manufacturer"] != '') ?
             getDropdownName("glpi_manufacturers", $fields["new_manufacturer"]) :
