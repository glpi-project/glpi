<?php
/*
 * @version $Id: calendar_holiday.class.php 11829 2010-06-29 11:56:38Z moyo $
 -------------------------------------------------------------------------
...
class Calendar_Holiday extends CommonDBRelation {
   ...
   static function showForCalendar(Calendar $calendar) {
      ...
      $rand=mt_rand();
      echo "<form name='calendarholiday_form$rand' id='calendarholiday_form$rand' method='post' action='";
      ...
      $query = "SELECT DISTINCT `glpi_calendars_holidays`.`id` AS linkID, `glpi_holidays`.*
                FROM `glpi_calendars_holidays`
                LEFT JOIN `glpi_holidays` ON (`glpi_calendars_holidays`.`holidays_id` = `glpi_holidays`.`id`)
