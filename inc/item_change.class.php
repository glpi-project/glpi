...
class Item_Change extends CommonDBRelation{
   ...
   static function countForItem(CommonDBTM $item) {
      ...
      $restrict = "`glpi_items_changes`.`changes_id` = `glpi_changes`.`id`
                   AND `glpi_documents_items`.`items_id` = '".$item->getField('id')."'
                   AND `glpi_documents_items`.`itemtype` = '".$item->getType()."'".
                   getEntitiesRestrictRequest(" AND ", "glpi_changes", '', '', true);

      $nb = countElementsInTable(array('glpi_items_changes', 'glpi_changes'), $restrict);
   ...
   **/
   static function showForChange(Change $change) {
      ...
      $query = "SELECT DISTINCT `itemtype`
                ...
                WHERE `glpi_items_changes`.`changes_id` = '$instID'
                ORDER BY `itemtype`";
      ...
      if ($canedit) {
         ...
         echo "<form method='post' name='itemchange_form$rand' id='itemchange_form$rand' action=\"".
                $CFG_GLPI["root_doc"]."/front/item_change.form.php\">";
      ...
      for ($i=0 ; $i<$number ; $i++) {
         ...
         if ($item->canView()) {
            ...
            $query .= " WHERE `$itemtable`.`id` = `glpi_items_changes`.`items_id`
                              AND `glpi_items_changes`.`itemtype` = '$itemtype'
                              AND `glpi_items_changes`.`changes_id` = '$instID'";
      ...
      if ($canedit) {
         ...
         openArrowMassive("itemchange_form$rand", true);
