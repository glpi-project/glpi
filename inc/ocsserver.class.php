...
class OcsServer extends CommonDBTM {
   ...
   **/
   function ocsFormConfig($target, $ID) {
      ...
      echo "<table width='100%'>";
      echo "<tr class='tab_bg_2'><td class='center'>"._n('Processor', 'Processors', 2)."</td>\n<td>";
      ...
      echo "<tr class='tab_bg_2'><td class='center'>"._n('Hard drive', 'Hard drives', 2)."</td>\n<td>";
      Dropdown::showYesNo("import_device_hdd", $this->fields["import_device_hdd"]);
      echo "</td></tr>\n";

      echo "<tr class='tab_bg_2'><td class='center'>"._n('Network card', 'Network cards', 2).
           "</td>\n<td>";
      Dropdown::showYesNo("import_device_iface", $this->fields["import_device_iface"]);
      echo "</td></tr>\n";

      echo "<tr class='tab_bg_2'><td class='center'>". _n('Graphics card', 'Graphics cards', 2).
           "</td>\n<td>";
      Dropdown::showYesNo("import_device_gfxcard", $this->fields["import_device_gfxcard"]);
      echo "&nbsp;&nbsp;</td></tr>";

      echo "<tr class='tab_bg_2'><td class='center'>"._n('Soundcard', 'Soundcards', 2)."</td>\n<td>";
      ...
      echo "<tr class='tab_bg_2'><td class='center'>" . _n('Modem','Modems',2) . " </td>\n<td>";
   ...
   **/
   function ocsFormImportOptions($target, $ID, $withtemplate='', $templateid='') {
      ...
      if ($this->fields['ocs_version'] > self::OCS1_3_VERSION_LIMIT) {
         echo "<tr class='tab_bg_2'>".
              "<td class='center'>"._n('Virtual machine', 'Virtual machines', 2). " </td>\n<td>";
   ...
   **/
   function showDBConnectionStatus($ID) {
      ...
      $out .="<tr><th>" .__('Connecting to the OCSNG database') . "</th></tr>\n";
      $out .="<tr class='tab_bg_2'><td class='center'>";
      if ($ID != -1) {
         if (!self::checkOCSconnection($ID)) {
            $out .= __('Connection to the OCSNG database failed');
            ...
            $out .= __('Access denied on OCSNG database (Need write rights on hardware.CHECKSUM necessary)');
   ...
   **/
   function updateAdminInfo(array $tab) {
   ...
   **/
    ...
    *
    * @param $id  integer  ID of the OCS config (default value 1)
    *
   ...
   **/
         ...
         if ($ocs_id_change
            ...
            if (!$ocs_id_change) {
               if ($cfg_ocs["import_general_os"]) {
                  self::resetDropdown($computers_id, "operatingsystems_id",
    ...
    * @param $ocsid
    * @param $ocsservers_id
    * @param $lock            (default 0)
    * @param $defaultentity   (default -1)
    * @param $defaultlocation (default -1)
   **/
   ...
   **/
   static function manageDeleted($ocsservers_id) {
      ...
      if (!(self::checkOCSconnection($ocsservers_id)
            && self::checkConfig(1))) {
         return false;
      }
   ...
   **/
   ...
   /**
    * @param $ocs_fields   array
    * @param $cfg_ocs
    * @param $entities_id
    * @param $locations_id       (default 0)
   **/
   ...
   **/
    ...
    * @param $ocsservers_id   integer : ocs server ID
    * @param $dohistory       boolean : do history ?
    * @param $force           boolean : force update ? (default 0)
    *
    * @return action done
   **/
      ...
      if ($DBocs->numrows($result) == 1) {
         ...
         if ($options['cfg_ocs']["import_general_os"]) {
            if (!in_array("operatingsystems_id", $options['computers_updates'])) {
               ...
               if (!$options['cfg_ocs']["ocs_db_utf8"]
                   && !Toolbox::seems_utf8($osname)) {
         ...
         if ($options['cfg_ocs']["import_general_comment"]
            ...
            $compupdate["comment"] = sprintf(__('%1$s %2$s'), $compupdate["comment"],
                                             sprintf(__('%1$s: %2$s'), __('Swap'), $line["SWAP"]));
         }
   ...
   **/
    ...
    * @param $dohistory          boolean  log changes ? (default 2)
    * @param entities_id                  the entity in which the computer is imported (default 0)
    *
    * @return nothing.
   **/
    ...
    * @param $entities_id  integer  entity in case of specific dropdown
    *
   ...
   **/
    ...
    *
    * @param $ocsservers_id   integer  id of ocs server in GLPI
    * @param $check           string   parameter for HTML input checkbox
    * @param $start           integer  parameter for Html::printPager method
    *
    * @return nothing
   **/
    ...
    * @return nothing
   **/
   static function cleanLinksFromList($ocsservers_id, array $ocslinks_id) {
      global $DB;
   ...
   **/
    ...
    * @param $todel
    * @param $field
    * @param $is_value_to_del          (false by default)
   **/
   ...
   **/
    ...
    *
    * @param $ocsservers_id   integer  the ID of the ocs server
    * @param $advanced                 display detail about the computer import or not
    *                                  (target entity, matched rules, etc.)
    ...
    *                                  (default 0)
    * @param $tolinked                 false for an import, true for a link (false by default)
    *
    * @return nothing
   **/
      ...
      if ($DBocs->numrows($result_ocs) > 0) {
         ...
         while ($data = $DBocs->fetch_assoc($result_ocs)) {
            ...
            foreach ($DBocs->request($query_network) as $network) {
               ...
               if (isset($network['MACADDR']) && ($network['MACADDR'] != '??')) {
   ...
   **/
   static function migrateImportDevice($computers_id, array $import_device) {
    ...
    * @param $import_device   array
   **/
   static function migrateComputerUpdates($computers_id, array $computer_update) {
                     ...
                     if (!empty($line2["CAPACITY"]) && ($line2["CAPACITY"] != "No")) {
                        ...
                        if ($line2["DESCRIPTION"]) {
                           if (!empty($ram["designation"])) {
                              $ram["designation"] = sprintf(__('%1$s - %2$s'), $ram["designation"],
                                                            $line2["DESCRIPTION"]);
                           } else {
            ...
            if ($cfg_ocs["import_device_port"]) {
               ...
               if ($DBocs->numrows($result2) > 0) {
                  while ($line2 = $DBocs->fetch_assoc($result2)) {
                     ...
                     $port["designation"] = "";
                     if ($line2["TYPE"] != "Other") {
                        $port["designation"] = sprintf(__('%1$s %2$s'),
...
//                                                                         $line2["TYPE"]);
         ...
         case self::SND_DEVICE :
            ...
            if ($cfg_ocs["import_device_sound"]) {
               ...
               if ($DBocs->numrows($result2) > 0) {
                  while ($line2 = $DBocs->fetch_assoc($result2)) {
                     $line2 = Toolbox::clean_cross_side_scripting_deep(Toolbox::addslashes_deep($line2));
                     if (!$cfg_ocs["ocs_db_utf8"]
                         && !Toolbox::seems_utf8($line2["NAME"])) {
    ...
    *
    * @param $ocsservers_id   integer  the ID of the OCS server
    * @param $ocsid           integer  ID of the computer in OCS hardware table
    * @param $todisplay                the link's label to display
    * @param $only_url                 (false by default)
    *
   ...
   **/
    ...
    *
    * @param ocsservers_id    integer  the ID of the OCS server
    * @param computers_id     integer  ID of the computer in OCS hardware table
    *
   ...
   **/
   static function checkOCSconnection($ocsservers_id) {
      ...
      if (!$DBocs
          || ($ocsservers_id != $DBocs->getServerID())) {
   ...
   **/
    ...
    *
    * @param $glpi_computers_id  integer  glpi computer id.
    * @param $field              string   of the computer table
   ...
   **/
    ...
    *
    * @param $software  integer  id of a software.
   ...
   **/
    ...
    * @param $dohistory
   **/
   static function updateVirtualMachines($computers_id, $ocsid, $ocsservers_id, $cfg_ocs,
                                         $import_vm, $dohistory) {
      global $DBocs;
    ...
    *@return Nothing (void).
   **/
   static function updateDisk($computers_id, $ocsid, $ocsservers_id, array $import_disk,
                              $dohistory) {
      global $DBocs;
   ...
   **/
    ...
    * @return Nothing (void).
   **/
   static function updateSoftware($computers_id, $entity, $ocsid, $ocsservers_id, array $cfg_ocs,
      ...
      if ($cfg_ocs["import_software"]) {
         ...
         if ($DBocs->numrows($result2) > 0) {
            while ($data2 = $DBocs->fetch_assoc($result2)) {
               ...
               // Hack for OCS encoding problems
               if (!$cfg_ocs["ocs_db_utf8"]
                   && !Toolbox::seems_utf8($initname)) {
                  $initname = Toolbox::encodeInUtf8($initname);
               }
               $name = $data2["NAME"];
               // Hack for OCS encoding problems
               if (!$cfg_ocs["ocs_db_utf8"]
                   && !Toolbox::seems_utf8($name)) {
                  $name = Toolbox::encodeInUtf8($name);
               }

               // Hack for OCS encoding problems
               if (!$cfg_ocs["ocs_db_utf8"]
                   && !Toolbox::seems_utf8($data2["PUBLISHER"])) {
    ...
    * @return Nothing (void).
   **/
   static function updateRegistry($computers_id, $ocsid, $ocsservers_id, array $cfg_ocs) {
      global $DB, $DBocs;
    ...
    * @return Nothing (void).
   **/
   static function updateAdministrativeInfo($computers_id, $ocsid, $ocsservers_id,
   ...
   **/
   static function updatePeripherals($itemtype, $entity, $computers_id, $ocsid, $ocsservers_id,
                                     array $cfg_ocs, array $import_periph, $dohistory) {
      global $DB, $DBocs;
      ...
      switch ($itemtype) {
         case 'Monitor' :
            if ($cfg_ocs["import_monitor"]) {
               ...
               $do_clean = true;
               $m        = new Monitor();
               $query    = "SELECT DISTINCT `CAPTION`, `MANUFACTURER`, `DESCRIPTION`, `SERIAL`,
                                            `TYPE`
                            FROM `monitors`
                            WHERE `HARDWARE_ID` = '$ocsid'";
               ...
               if ((count($lines) > 0)
                  ...
                  foreach ($lines as $line) {
                     ...
                     if (empty($line["CAPTION"])
                         && !empty($line["MANUFACTURER"])) {
                        $mon["name"] = $line["MANUFACTURER"];
                     }
                     if (empty($line["CAPTION"])
                         && !empty($line["TYPE"])) {
                        if (!empty($line["MANUFACTURER"])) {
                           $mon["name"] = sprintf(__('%1$s %2$s'), $mon["name"], $line["TYPE"]);
                        } else {
                           $mon["name"] = $line["TYPE"];
                        }
                     }
                     ...
                     if (!empty($mon["serial"])) {
                        $checkMonitor = sprintf(__('%1$s %2$s'), $checkMonitor, $mon["serial"]);
                     }
                     if (!empty($mon["name"])) {
...
//                                  }
   ...
   **/
   ...
   /**
    * @param $statistics   array
    * @param $finished           (false by default)
   **/
    ...
    * @return nothing
   **/
   static function transferComputer(array $line_links) {
      global $DB, $DBocs, $CFG_GLPI;
      ...
      if (isset($data['entities_id'])
         ...
         if (!isCommandLine()
             && !Session::haveAccessToEntity($data['entities_id'])) {
   ...
   **/
   static function updateTag(array $line_links) {
      global $DB, $DBocs;
