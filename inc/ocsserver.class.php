...
class OcsServer extends CommonDBTM {
      ...
      if (!$ocs_exists || $numrows == 0) {
         ...
         if ($idlink = OcsServer::ocsLink($ocsid, $ocsservers_id, $computers_id)) {
            ...
            if ($cfg_ocs["import_device_processor"]) {
               OcsServer::resetDevices($computers_id, PROCESSOR_DEVICE);
            }
            if ($cfg_ocs["import_device_iface"]) {
               OcsServer::resetDevices($computers_id, NETWORK_DEVICE);
            }
            if ($cfg_ocs["import_device_memory"]) {
               OcsServer::resetDevices($computers_id, RAM_DEVICE);
            }
            if ($cfg_ocs["import_device_hdd"]) {
               OcsServer::resetDevices($computers_id, HDD_DEVICE);
            }
            if ($cfg_ocs["import_device_sound"]) {
               OcsServer::resetDevices($computers_id, SND_DEVICE);
            }
            if ($cfg_ocs["import_device_gfxcard"]) {
               OcsServer::resetDevices($computers_id, GFX_DEVICE);
            }
            if ($cfg_ocs["import_device_drive"]) {
               OcsServer::resetDevices($computers_id, DRIVE_DEVICE);
            }
            if ($cfg_ocs["import_device_modem"] || $cfg_ocs["import_device_port"]) {
               OcsServer::resetDevices($computers_id, PCI_DEVICE);
            }
   ...
   static function importComputer($ocsid, $ocsservers_id, $lock = 0, $defaultentity = -1,$canlink=0) {
      ...
      if (isset ($data['entities_id']) && $data['entities_id'] >= 0) {
         ...
         if ($canlink) {
            $found_computers = OcsServer::getComputersAlreadyImported($ocsid,$ocsservers_id,$data['entities_id']);
   ...
   */
      ...
      if ($DB->numrows($result) == 1) {
         ...
         if ($DBocs->numrows($result_ocs) == 1) {
            ...
            if ($mixed_checksum) {
               ...
               $computer_updates = OcsServer::importArrayFromDB($line["computer_update"]);
               if (!in_array(OCS_IMPORT_TAG_080,$computer_updates)) {
                  $computer_updates=OcsServer::migrateComputerUpdates($line["computers_id"],$computer_updates);
               }
               // Update Administrative informations
               OcsServer::updateAdministrativeInfo($line['computers_id'], $line['ocsid'], $ocsservers_id,
                                           $cfg_ocs, $computer_updates, $comp->fields['entities_id'],
                                           $dohistory);
               if ($mixed_checksum & pow(2, HARDWARE_FL)) {
                  $loghistory = OcsServer::updateHardware($line['computers_id'], $line['ocsid'], $ocsservers_id,
                                                  $cfg_ocs, $computer_updates, $dohistory);
               }
               if ($mixed_checksum & pow(2, BIOS_FL)) {
                  OcsServer::updateBios($line['computers_id'], $line['ocsid'], $ocsservers_id, $cfg_ocs,
               ...
               if ($mixed_checksum & pow(2, MEMORIES_FL)) {
                  OcsServer::updateDevices(RAM_DEVICE, $line['computers_id'], $line['ocsid'], $ocsservers_id,
                                   $cfg_ocs, $import_device, '', $dohistory);
               }
               if ($mixed_checksum & pow(2, STORAGES_FL)) {
                  OcsServer::updateDevices(HDD_DEVICE, $line['computers_id'], $line['ocsid'], $ocsservers_id,
                                   $cfg_ocs, $import_device, '', $dohistory);
                  OcsServer::updateDevices(DRIVE_DEVICE, $line['computers_id'], $line['ocsid'], $ocsservers_id,
                                   $cfg_ocs, $import_device, '', $dohistory);
               }
               if ($mixed_checksum & pow(2, HARDWARE_FL)) {
                  OcsServer::updateDevices(PROCESSOR_DEVICE, $line['computers_id'], $line['ocsid'],
                                   $ocsservers_id, $cfg_ocs, $import_device, '', $dohistory);
               }
               if ($mixed_checksum & pow(2, VIDEOS_FL)) {
                  OcsServer::updateDevices(GFX_DEVICE, $line['computers_id'], $line['ocsid'], $ocsservers_id,
                                   $cfg_ocs, $import_device, '', $dohistory);
               }
               if ($mixed_checksum & pow(2, SOUNDS_FL)) {
                  OcsServer::updateDevices(SND_DEVICE, $line['computers_id'], $line['ocsid'], $ocsservers_id,
                                   $cfg_ocs, $import_device, '', $dohistory);
               }
               ...
               if ($mixed_checksum & pow(2, MODEMS_FL) || $mixed_checksum & pow(2, PORTS_FL)) {
                  OcsServer::updateDevices(PCI_DEVICE, $line['computers_id'], $line['ocsid'], $ocsservers_id,
                                   $cfg_ocs, $import_device, '', $dohistory);
               }
               if ($mixed_checksum & pow(2, MONITORS_FL)) {
                  // Get import monitors
                  $import_monitor = OcsServer::importArrayFromDB($line["import_monitor"]);
                  OcsServer::updatePeripherals(MONITOR_TYPE, $comp->fields["entities_id"], $line['computers_id'],
               ...
               if ($mixed_checksum & pow(2, PRINTERS_FL)) {
                  ...
                  OcsServer::updatePeripherals(PRINTER_TYPE, $comp->fields["entities_id"], $line['computers_id'],
               ...
               if ($mixed_checksum & pow(2, INPUTS_FL)) {
                  ...
                  OcsServer::updatePeripherals(PERIPHERAL_TYPE, $comp->fields["entities_id"],
               ...
               if ($mixed_checksum & pow(2, SOFTWARES_FL)) {
                  ...
                  OcsServer::updateSoftware($line['computers_id'], $comp->fields["entities_id"], $line['ocsid'],
               ...
               if ($mixed_checksum & pow(2, DRIVES_FL)) {
                  ...
                  OcsServer::updateDisk($line['computers_id'], $line['ocsid'], $ocsservers_id, $cfg_ocs,
                                $import_disk);
               }
               if ($mixed_checksum & pow(2, REGISTRY_FL)) {
                  //import registry entries not needed
                  OcsServer::updateRegistry($line['computers_id'], $line['ocsid'], $ocsservers_id, $cfg_ocs);
               }
    ...
    *
    **/
   static function updateHardware($computers_id, $ocsid, $ocsservers_id, $cfg_ocs, $computer_updates,
                              $dohistory = 2) {
    ...
    *
    **/
   function updateBios($computers_id, $ocsid, $ocsservers_id, $cfg_ocs, $computer_updates,
                          $dohistory = 2) {
      global $DBocs;
   ...
   static function mergeOcsArray($computers_id, $tomerge, $field) {
      ...
      if ($result = $DB->query($query)) {
         if ($DB->numrows($result)) {
            $tab = OcsServer::importArrayFromDB($DB->result($result, 0, 0));
            $newtab = array_merge($tomerge, $tab);
            $newtab = array_unique($newtab);
            $query = "UPDATE
                      `glpi_ocslinks`
                      SET `$field` = '" . OcsServer::exportArrayToDB($newtab) . "'
                      WHERE `computers_id` = '$computers_id'";
            $DB->query($query);
         }
      }
      ...
      if ($result = $DB->query($query)) {
         if ($DB->numrows($result)) {
            ...
            if (isset($tab[$todel])) {
               ...
               $query = "UPDATE
                         `glpi_ocslinks`
                         SET `$field` = '" . OcsServer::exportArrayToDB($tab) . "'
                         WHERE `computers_id` = '$computers_id'";
               $DB->query($query);
            }
         }
   ...
   static function replaceOcsArray($computers_id, $newArray, $field) {
      ...
      $newArray = OcsServer::exportArrayToDB($newArray);
      $query = "SELECT `$field`
                FROM `glpi_ocslinks`
                WHERE `computers_id` = '$computers_id'";
   ...
   static function ocsShowNewComputer($ocsservers_id, $advanced, $check, $start, $entity=0, $tolinked=false) {
      global $DB, $DBocs, $LANG, $CFG_GLPI;
   ...
   static function editLock($target, $ID) {
      ...
      if ($DB->numrows($result) == 1) {
         ...
         $locked = OcsServer::importArrayFromDB($data["computer_update"]);
         ...
         $locked_monitor = OcsServer::importArrayFromDB($data["import_monitor"]);
         ...
         $header = false;
         echo "<br><div class='center'>";
         $locked_printer = OcsServer::importArrayFromDB($data["import_printer"]);
         ...
         $header = false;
         echo "<br><div class='center'>";
         $locked_printer = OcsServer::importArrayFromDB($data["import_peripheral"]);
         foreach ($locked_printer as $key => $val) {
         ...
         $header = false;
         echo "<br><div class='center'>";
         $locked_ip = OcsServer::importArrayFromDB($data["import_ip"]);
         ...
         $header = false;
         echo "<br><div class='center'>";
         $locked_software = OcsServer::importArrayFromDB($data["import_software"]);
         ...
         $header = false;
         echo "<br><div class='center'>";
         $locked = OcsServer::importArrayFromDB($data["import_disk"]);
         foreach ($locked as $key => $val) {
            $querySearchLocked = "SELECT `id`
                                  FROM `glpi_computerdisks`
                                  WHERE `id` = '$key'";
            ...
            if ($cfg_ocs["import_device_memory"]) {
               ...
               if ($DBocs->numrows($result2) > 0) {
                  ...
                  if (!in_array(OCS_IMPORT_TAG_072,$import_device)) {
                     OcsServer::addToOcsArray($computers_id,array(0=>OCS_IMPORT_TAG_072),"import_device");
                  ...
                  while ($line2 = $DBocs->fetch_array($result2)) {
                     ...
                     if (!empty ($line2["CAPACITY"]) && $line2["CAPACITY"] != "No") {
                        ...
                        if (!in_array(RAM_DEVICE . OCS_FIELD_SEPARATOR . $ram["designation"],
                           ...
                           $ram_id = OcsServer::addDevice(RAM_DEVICE, $ram);
                           if ($ram_id) {
                  ...
                  while ($line2 = $DBocs->fetch_array($result2)) {
                     ...
                     if (!empty ($line2["DISKSIZE"]) && preg_match("/disk/i", $line2["TYPE"])) {
                        ...
                        if (!in_array(HDD_DEVICE . OCS_FIELD_SEPARATOR . $dd["designation"],
                           ...
                           $dd_id = OcsServer::addDevice(HDD_DEVICE, $dd);
                           if ($dd_id) {
                  ...
                  while ($line2 = $DBocs->fetch_array($result2)) {
                     ...
                     if (empty ($line2["DISKSIZE"]) || !preg_match("/disk/i", $line2["TYPE"])) {
                        ...
                        if (!in_array(DRIVE_DEVICE . OCS_FIELD_SEPARATOR . $stor["designation"],
                           ...
                           $stor_id = OcsServer::addDevice(DRIVE_DEVICE, $stor);
                           if ($stor_id) {
               ...
               if ($DBocs->numrows($result2) > 0) {
                  while ($line2 = $DBocs->fetch_array($result2)) {
                     ...
                     if (!in_array(PCI_DEVICE . OCS_FIELD_SEPARATOR . $mdm["designation"],
                        ...
                        $mdm_id = OcsServer::addDevice(PCI_DEVICE, $mdm);
                        if ($mdm_id) {
                  ...
                  while ($line2 = $DBocs->fetch_array($result2)) {
                     ...
                     if (!empty ($port["designation"])) {
                        if (!in_array(PCI_DEVICE . OCS_FIELD_SEPARATOR . $port["designation"],
                           ...
                           $port_id = OcsServer::addDevice(PCI_DEVICE, $port);
                           if ($port_id) {
               ...
               if ($DBocs->numrows($result) == 1) {
                  ...
                  for ($i = 0; $i < $line["PROCESSORN"]; $i++) {
                     ...
                     if (!in_array(PROCESSOR_DEVICE . OCS_FIELD_SEPARATOR . $processor["designation"],
                                   $import_device)) {
                        $proc_id = OcsServer::addDevice(PROCESSOR_DEVICE, $processor);
                        if ($proc_id) {
                  ...
                  while ($line2 = $DBocs->fetch_array($result2)) {
                     ...
                     if ($cfg_ocs["import_device_iface"]) {
                        ...
                        if (!in_array(NETWORK_DEVICE . OCS_FIELD_SEPARATOR . $network["designation"],
                           ...
                           $net_id = OcsServer::addDevice(NETWORK_DEVICE, $network);
                           if ($net_id) {
               ...
               if ($DBocs->numrows($result2) > 0) {
                  while ($line2 = $DBocs->fetch_array($result2)) {
                     ...
                     if (!in_array(GFX_DEVICE . OCS_FIELD_SEPARATOR . $video["designation"],
                        ...
                        $video_id = OcsServer::addDevice(GFX_DEVICE, $video);
                        if ($video_id) {
               ...
               if ($DBocs->numrows($result2) > 0) {
                  while ($line2 = $DBocs->fetch_array($result2)) {
                     ...
                     if (!in_array(SND_DEVICE . OCS_FIELD_SEPARATOR . $snd["designation"],
                        ...
                        $snd_id = OcsServer::addDevice(SND_DEVICE, $snd);
                        if ($snd_id) {
    ...
    *
    **/
   static function exportArrayToDB($TAB) {
    ...
    *
    **/
   static function importArrayFromDB($DATA) {
   ...
   static function migrateImportIP($computers_id,$import_ip) {
      ...
      //Add the new tag as the first occurence in the array
      OcsServer::addToOcsArray($computers_id,array(0=>OCS_IMPORT_TAG_072),"import_ip");
      ...
      if (count($import_ip) > 1) {
         foreach($import_ip as $importip_ID => $value) {
            if ($importip_ID > 0) {
               ...
               OcsServer::addToOcsArray($computers_id,array($importip_ID=>$new_ip . OCS_FIELD_SEPARATOR . $new_mac),
                             "import_ip");
      ...
      if ($cfg_ocs["import_software"]) {
         ...
         if (!in_array(OCS_IMPORT_TAG_070, $import_software)) {
            ...
            if ($DB->numrows($result)) {
               ...
               $import_software = OcsServer::importArrayFromDB($tmp["import_software"]);
            }
         }
            ...
            if ($cfg_ocs["import_monitor"]) {
               ...
               if (!in_array(OCS_IMPORT_TAG_070, $import_periph)) {
                  foreach ($import_periph as $key => $val) {
                     ...
                     OcsServer::addToOcsArray($computers_id, array ($key => $monitor_tag), "import_monitor");
                     //Update the array with the new value of the monitor
                     $import_periph[$key] = $monitor_tag;
                  }
                  //add the tag for the array version's
                  OcsServer::addToOcsArray($computers_id, array(0 => OCS_IMPORT_TAG_070), "import_monitor");
               }
               $do_clean = true;
               $m = new Monitor;
                  ...
                  foreach ($lines as $line) {
                     ...
                     if ($id === false) {
                        ...
                        if ($id_monitor) {
                           ...
                           if (!in_array(OCS_IMPORT_TAG_070, $import_periph)) {
                              OcsServer::addToOcsArray($computers_id, array (0 => OCS_IMPORT_TAG_070),
                                            "import_monitor");
                           }
    ...
    *
    **/
   static function addDevice($devicetype, $dev_array) {
      global $DB;
