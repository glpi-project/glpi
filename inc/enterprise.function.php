   ...
   if ($canedit) {
      if ($enterprise->fields["is_recursive"]) {
         $nb=countElementsInTableForEntity("glpi_contacts",
               getSonsOf("glpi_entities",$enterprise->fields["entities_id"]));
      } else {
      ...
      if ($nb>count($used)) {
         ...
         if ($enterprise->fields["is_recursive"]) {
            dropdown("glpi_contacts","contacts_id",1,
                     getSonsOf("glpi_entities",$enterprise->fields["entities_id"]),$used);
         } else {
...
function getEnterpriseLinks($value,$withname=false) {
   ...
   if ($ent->getFromDB($value)) {
      ...
      if (!empty($ent->fields['website'])) {
         $ret.= "&nbsp;&nbsp;";
         $ret.= "<a href='".formatOutputWebLink($ent->fields['website'])."' target='_blank'>
