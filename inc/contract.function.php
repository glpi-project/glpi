...
function showEnterpriseContract($instID) {
	...
	if ($canedit){
		if ($contract->fields["recursive"]) {
			$nb=countElementsInTableForMyEntities("glpi_contacts");			
		} else {
...
function showContractAssociatedEnterprise($ID){
	...
	while ($data=$DB->fetch_array($result)) {
		...
		echo "<td align='center' class='tab_bg_2'>";
		if ($canedit) 
			echo "<a href='".$CFG_GLPI["root_doc"]."/front/enterprise.form.php?deletecontract=deletecontract&amp;ID=$assocID'><strong>".$LANG["buttons"][6]."</strong></a>";
		else echo "&nbsp;";
		echo "</td></tr>";
		$i++;
	}
	if ($canedit){
		if ($ent->fields["recursive"]) {
			$nb=countElementsInTableForMyEntities("glpi_contracts");			
		} else {
		...
		if ($nb>count($used)){
			...
			if ($ent->fields["recursive"]) {
				dropdownContracts("conID",-1,$used);
			} else {
