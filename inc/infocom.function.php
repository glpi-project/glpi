   ...
   if ($ci->getFromDB($itemtype,$dev_ID)) {
      ...
      if (!$ic->getFromDBforDevice($itemtype,$dev_ID)) {
         ...
         if ($show_immo) {
            ...
            echo formatNumber(TableauAmort($ic->fields["sink_type"],$ic->fields["value"],
...
function showDisplayInfocomLink($itemtype,$device_id,$update=0) {
   ...
   if (haveTypeRight($itemtype,"w")) {
      echo "<span onClick=\"window.open('".$CFG_GLPI["root_doc"].
            "/front/infocom.show.php?itemtype=$itemtype&amp;device_id=$device_id&amp;update=$update',
      ...
      while ($data=$DB->fetch_array($result)) {
         if ($ci->getFromDB($data["itemtype"],$data["items_id"])) {
            ...
            if (!$ci->getField('is_template')) {
               $message[$entity].=$LANG['mailing'][40]." ".$ci->getType()." - ".$ci->getName().": ".
