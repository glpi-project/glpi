...
**/
   function prepareInputForUpdate($input) {
      ...
      if (isset($input["import_ip"])) {
         ...
         if ($input["import_device_port"]) {
            $input["checksum"]|= pow(2,PORTS_FL);
         }
         if ($input["import_device_modem"]) {
            $input["checksum"]|= pow(2,MODEMS_FL);
         }
         if ($input["import_device_drive"]) {
            $input["checksum"]|= pow(2,STORAGES_FL);
         }
         if ($input["import_device_sound"]) {
            $input["checksum"]|= pow(2,SOUNDS_FL);
         }
         if ($input["import_device_gfxcard"]) {
            $input["checksum"]|= pow(2,VIDEOS_FL);
         }
         if ($input["import_device_iface"]) {
            $input["checksum"]|= pow(2,NETWORKS_FL);
         }
         if ($input["import_device_hdd"]) {
            $input["checksum"]|= pow(2,STORAGES_FL);
         }
