...
class ITILCategory extends CommonTreeDropdown {
   ...
   function getAdditionalFields() {
      ...
      $tab = [['name'      => $this->getForeignKeyField(),
                   ...
                   ['name'      => 'knowbaseitemcategories_id',
                         ...
                         'list'      => true],
                   ['name'      => 'code',
                         'label'     => __('Code representing the ticket categorie'),
                         'type'      => 'text',
                         'list'      => false],
   ...
   function rawSearchOptions() {
      ...
      $tab[] = [
         ...
         'table'              => $this->getTable(),
         'field'              => 'code',
         'name'               => __('Code representing the ticket categorie'),
         'massiveaction'      => false,
         'datatype'           => 'string'
      ];
   ...
   **/
   function prepareInputForAdd($input) {
      ...
      if (!empty($input["code"]) 
         ...
         Session::addMessageAfterRedirect(__("Code representing the ticket categorie is already used"),
                                          false, ERROR);
         return false;
      }
      return $input;
   ...
   **/
   function prepareInputForUpdate($input) {
      ...
      if (!empty($input["code"]) 
            && ITILCategory::getITILCategoryIDByCode($input["code"]) != -1
            && ITILCategory::getITILCategoryIDByCode($input["code"]) != $input["id"]) {
         Session::addMessageAfterRedirect(__("Code representing the ticket categorie is already used"),
                                          false, ERROR);
         return false;
      }
