...
function update05to051() {
   ...
   if (!TableExists("glpi_dropdown_contact_type")) {
      ...
      $DB->query($query)
      or die("0.51 add table dropdown_contact_type ".$LANG['update'][90].$DB->error());

      $query = "INSERT INTO `glpi_dropdown_contact_type`
                       (`name`)
                VALUES ('".$LANG['financial'][43]."')";
      $DB->query($query)
      or die("0.51 add entries to dropdown_contact_type ".$LANG['update'][90].$DB->error());
   ...
   if (!TableExists("glpi_state_item")) {
      ...
      $DB->query($query)
      or die("0.51 alter glpi_state_item table name ".$LANG['update'][90].$DB->error());
