...
function add_tracking($type,$ID){
	...
	while (mt_rand(0,100)<$percent['tracking_on_item']){
		...
		$query="INSERT INTO glpi_tracking VALUES (NULL,'".date("Y-m-d H:i:s",$date1)."','".date("Y-m-d H:i:s",$date2)."','$status','".$users[0]."','".$users[1]."','$enterprise','$type','$ID','tracking ".GetRandomString(15)."','".mt_rand(1,5)."','no','','no','".(mt_rand(0,3)+mt_rand(0,100)/100)."','".mt_rand(1,$max['tracking_category'])."','0','0','0')";
		$db->query($query) or die("PB REQUETE ".$query);
		$tID=$db->insert_id();
		// Add followups
		$i=0;
...
for ($i=0;$i<max(1,pow($max['kbcategories'],1/3));$i++){
	...
	for ($j=0;$j<mt_rand(0,pow($max['kbcategories'],1/2));$j++){
		...
		for ($k=0;$k<mt_rand(0,pow($max['kbcategories'],1/2));$k++){
			$query="INSERT INTO glpi_dropdown_kbcategories VALUES (NULL,'$newID2','ss-categorie $k','','comment categorie $i','3')";
			$db->query($query) or die("PB REQUETE ".$query);
		}	
	}
}	
...
for ($i=0;$i<$max['contract'];$i++){
	...
	$query="INSERT INTO glpi_contracts VALUES (NULL,'contract $i','num $i','".mt_rand(100,10000)."','".mt_rand(1,$MAX_CONTRACT_TYPE)."','$date','".mt_rand(1,36)."','".mt_rand(1,3)."','".mt_rand(1,6)."','".mt_rand(1,12)."','".mt_rand(1,6)."','comment $i','compta num $i','N','08:00:00','19:00:00','09:00:00','16:00:00','Y','00:00:00','00:00:00','N','0','notes contract $i')";
	$db->query($query) or die("PB REQUETE ".$query);
	$conID=$db->insert_id();
	add_documents(CONTRACT_TYPE,$conID);
	// Add an enterprise
	$query="INSERT INTO glpi_contract_enterprise VALUES(NULL,'".mt_rand(1,$max["enterprises"])."','$conID');";
	$db->query($query) or die("PB REQUETE ".$query);
}
...
for ($i=0;$i<pow($max['locations'],1/5);$i++){
	$query="INSERT INTO glpi_dropdown_locations VALUES (NULL,'lieu $i','0','','comment lieu $i','1')";
	$db->query($query) or die("PB REQUETE ".$query);
	$newID=$db->insert_id();
	for ($j=0;$j<mt_rand(0,pow($max['locations'],1/4));$j++){
		$query="INSERT INTO glpi_dropdown_locations VALUES (NULL,'s-lieu $j','$newID','','comment s-lieu $j','2')";
		$db->query($query) or die("PB REQUETE ".$query);
		$newID2=$db->insert_id();
		for ($k=0;$k<mt_rand(0,pow($max['locations'],1/4));$k++){
			$query="INSERT INTO glpi_dropdown_locations VALUES (NULL,'ss-lieu $k','$newID2','','comment ss-lieu $k','3')";
			$db->query($query) or die("PB REQUETE ".$query);
			$newID3=$db->insert_id();
			for ($l=0;$l<mt_rand(0,pow($max['locations'],1/4));$l++){
				$query="INSERT INTO glpi_dropdown_locations VALUES (NULL,'sss-lieu $l','$newID3','','comment sss-lieu $l','4')";
				$db->query($query) or die("PB REQUETE ".$query);
				$newID4=$db->insert_id();
				for ($m=0;$m<mt_rand(0,pow($max['locations'],1/4));$m++){
					$query="INSERT INTO glpi_dropdown_locations VALUES (NULL,'ssss-lieu $m','$newID4','','comment ssss-lieu $m',5)";
					$db->query($query) or die("PB REQUETE ".$query);
				}	
			}	
		}	
...
for ($i=0;$i<$max['users_sadmin'];$i++){
	...
	$db->query($query) or die("PB REQUETE ".$query);
	$query="INSERT INTO glpi_users_profiles VALUES (NULL,'".$db->insert_id()."','4');";
	$db->query($query) or die("PB REQUETE ".$query);
}
for ($i=0;$i<$max['users_admin'];$i++){
	...
	$db->query($query) or die("PB REQUETE ".$query);
	$query="INSERT INTO glpi_users_profiles VALUES (NULL,'".$db->insert_id()."','3');";
	$db->query($query) or die("PB REQUETE ".$query);
}
for ($i=0;$i<$max['users_normal'];$i++){
	...
	$db->query($query) or die("PB REQUETE ".$query);
	$query="INSERT INTO glpi_users_profiles VALUES (NULL,'".$db->insert_id()."','2');";
	$db->query($query) or die("PB REQUETE ".$query);
}
for ($i=0;$i<$max['users_postonly'];$i++){
	...
	$db->query($query) or die("PB REQUETE ".$query);
	$query="INSERT INTO glpi_users_profiles VALUES (NULL,'".$db->insert_id()."','1');";
	$db->query($query) or die("PB REQUETE ".$query);
}
...
for ($i=0;$i<$max['type_of_consumables'];$i++){
	$query="INSERT INTO glpi_consumables_type VALUES (NULL,'consumable type $i','ref $i','".mt_rand(1,$max['locations'])."','".mt_rand(1,$max['consumable_type'])."','".mt_rand(1,$max['enterprises'])."','".mt_rand(1,$max['users_sadmin']+$max['users_admin'])."','N','comments $i','".mt_rand(0,10)."','notes consumable type $i')";
	$db->query($query) or die("PB REQUETE ".$query);
	$consID=$db->insert_id();
	...
	for ($j=0;$j<mt_rand(0,$max['consumables_stock']);$j++){
		...
		// AJOUT INFOCOMS
		$query="INSERT INTO glpi_infocoms VALUES (NULL,'$ID','".CONSUMABLE_ITEM_TYPE."','$date','$date','".mt_rand(12,36)."','infowar cons $ID','".mt_rand(1,$max['enterprises'])."','commande cons $ID','BL cart $ID','immo cons $ID','".mt_rand(0,5000)."','".mt_rand(0,500)."','".mt_rand(1,7)."','".mt_rand(1,2)."','".mt_rand(2,5)."','comments cons $ID','facture cons $ID','".mt_rand(1,$max['budget'])."')";
		$db->query($query) or die("PB REQUETE ".$query);
	}
...
for ($i=0;$i<$max['type_of_cartridges'];$i++){
	$query="INSERT INTO glpi_cartridges_type VALUES (NULL,'cartridge type $i','ref $i','".mt_rand(1,$max['locations'])."','".mt_rand(1,$max['cartridge_type'])."','".mt_rand(1,$max['enterprises'])."','".mt_rand(1,$max['users_sadmin']+$max['users_admin'])."','N','comments $i','".mt_rand(0,10)."','notes cartridges type $i')";
	$db->query($query) or die("PB REQUETE ".$query);
	$cartID=$db->insert_id();
	...
	// AJOUT INFOCOMS
	$date=mt_rand(1995,2005)."-".mt_rand(1,12)."-".mt_rand(1,28);
	$query="INSERT INTO glpi_infocoms VALUES (NULL,'$ID','".CARTRIDGE_ITEM_TYPE."','$date','$date','".mt_rand(12,36)."','infowar cart $ID','".mt_rand(1,$max['enterprises'])."','commande cart $ID','BL cart $ID','immo cart $ID','".mt_rand(0,5000)."','".mt_rand(0,500)."','".mt_rand(1,7)."','".mt_rand(1,2)."','".mt_rand(2,5)."','comments cart $ID','facture cart $ID','".mt_rand(1,$max['budget'])."')";
	$db->query($query) or die("PB REQUETE ".$query);
...
for ($i=0;$i<$max['type_printers'];$i++){
	$query="INSERT INTO glpi_cartridges_assoc VALUES (NULL,'".mt_rand(1,$max['type_of_cartridges'])."','$i')";
	$db->query($query) or die("PB REQUETE ".$query);
}
...
while ($data=$db->fetch_array($result)){
	...
	$query="INSERT INTO glpi_printers VALUES (NULL,'printer of loc ".$data['ID']."',NOW(),'contact ".$data['ID']."','num ".$data['ID']."','$techID','".GetRandomString(10)."','".GetRandomString(10)."','0','0','1','comments $i','".mt_rand(0,64)."','".$data['ID']."','$domainID','$networkID','$modelID','$typeID','".mt_rand(1,$max['enterprises'])."','0','N','0','','0','notes printers ".$data['ID']."')";
	$db->query($query) or die("PB REQUETE ".$query);
...
for ($i=0;$i<$max['device'];$i++){
	...
	$query="INSERT INTO glpi_device_case VALUES (NULL,'$val','".mt_rand(0,2)."','comment $i','".mt_rand(1,$max['enterprises'])."','".mt_rand(0,10)."')";
	$db->query($query) or die("PB REQUETE ".$query);
}
...
for ($i=0;$i<$max['device'];$i++){
	...
	$query="INSERT INTO glpi_device_control VALUES (NULL,'$val','".mt_rand(0,3)."','N','comment $i','".mt_rand(1,$max['enterprises'])."','".mt_rand(0,1000)."')";
	$db->query($query) or die("PB REQUETE ".$query);
}
...
for ($i=0;$i<$max['device'];$i++){
	...
	$query="INSERT INTO glpi_device_drive VALUES (NULL,'$val','Y','".mt_rand(0,60)."','".mt_rand(0,2)."','comment $i','".mt_rand(1,$max['enterprises'])."','".mt_rand(0,100)."')";
	$db->query($query) or die("PB REQUETE ".$query);
}
...
for ($i=0;$i<$max['device'];$i++){
	...
	$query="INSERT INTO glpi_device_gfxcard VALUES (NULL,'$val','".mt_rand(0,128)."','".mt_rand(0,3)."','comment $i','".mt_rand(1,$max['enterprises'])."','".mt_rand(0,128)."')";
	$db->query($query) or die("PB REQUETE ".$query);
}
...
for ($i=0;$i<$max['device'];$i++){
	...
	$query="INSERT INTO glpi_device_hdd VALUES (NULL,'$val','".mt_rand(0,10500)."','".mt_rand(1,$max['hdd_type'])."','".mt_rand(0,8000)."','comment $i','".mt_rand(1,$max['enterprises'])."','".mt_rand(0,128)."')";
	$db->query($query) or die("PB REQUETE ".$query);
}
...
for ($i=0;$i<$max['device'];$i++){
	...
	$query="INSERT INTO glpi_device_iface VALUES (NULL,'$val','".mt_rand(0,1000)."','comment $i','".mt_rand(1,$max['enterprises'])."','".mt_rand(0,10)."')";
	$db->query($query) or die("PB REQUETE ".$query);
}
...
for ($i=0;$i<$max['device'];$i++){
	...
	$query="INSERT INTO glpi_device_moboard VALUES (NULL,'$val','chipset ".mt_rand(0,1000)."','comment $i','".mt_rand(1,$max['enterprises'])."','".mt_rand(0,10)."')";
	$db->query($query) or die("PB REQUETE ".$query);
}
...
for ($i=0;$i<$max['device'];$i++){
	...
	$query="INSERT INTO glpi_device_pci VALUES (NULL,'$val','comment $i','".mt_rand(1,$max['enterprises'])."','".mt_rand(0,10)."')";
	$db->query($query) or die("PB REQUETE ".$query);
}
...
for ($i=0;$i<$max['device'];$i++){
	...
	$query="INSERT INTO glpi_device_power VALUES (NULL,'$val','".mt_rand(0,500)."W','Y','comment $i','".mt_rand(1,$max['enterprises'])."','".mt_rand(0,10)."')";
	$db->query($query) or die("PB REQUETE ".$query);
}
...
for ($i=0;$i<$max['device'];$i++){
	...
	$query="INSERT INTO glpi_device_processor VALUES (NULL,'$val','".mt_rand(1000,3000)."','comment $i','".mt_rand(1,$max['enterprises'])."','".mt_rand(1000,3000)."')";
	$db->query($query) or die("PB REQUETE ".$query);
}
...
for ($i=0;$i<$max['device'];$i++){
	...
	$query="INSERT INTO glpi_device_ram VALUES (NULL,'$val','".mt_rand(0,400)."','comment $i','".mt_rand(1,$max['enterprises'])."','".mt_rand(0,10)."','".mt_rand(1,$max['ram_type'])."')";
	$db->query($query) or die("PB REQUETE ".$query);
}
...
for ($i=0;$i<$max['device'];$i++){
	...
	$query="INSERT INTO glpi_device_sndcard VALUES (NULL,'$val','type ".mt_rand(0,100)."','comment $i','".mt_rand(1,$max['enterprises'])."','".mt_rand(0,100)."')";
	$db->query($query) or die("PB REQUETE ".$query);
}
...
for ($i=0;$i<$max['computers'];$i++){
	...
	$domainID=mt_rand(1,$max['domain']);
	$networkID=mt_rand(1,$max['network']);
	$query="INSERT INTO glpi_computers VALUES (NULL,'computers $i','".GetRandomString(10)."','".GetRandomString(10)."','contact $i','num $i','$techID','',NOW(),'".mt_rand(1,$max['os'])."','".mt_rand(1,$max['os_version'])."','".mt_rand(1,$max['os_sp'])."','".mt_rand(1,$max['auto_update'])."','".$loc."','$domainID','$networkID','".mt_rand(1,$max['model'])."','".mt_rand(1,$max['type_computers'])."','0','','".mt_rand(1,$max['enterprises'])."','N','note computer $i','0')";
	$db->query($query) or die("PB REQUETE ".$query);
	$compID=$db->insert_id();
	...
	$query="INSERT INTO glpi_phones VALUES (NULL,'phone $i',NOW(),'contact $i','num $i','$techID','comment $i','".GetRandomString(10)."','".GetRandomString(10)."','".GetRandomString(10)."','$loc','".mt_rand(1,$max['type_phones'])."','".mt_rand(1,$max['model_phones'])."','".GetRandomString(10)."','".mt_rand(1,$max['phone_power'])."','".GetRandomString(10)."','".mt_rand(0,1)."','".mt_rand(0,1)."','".mt_rand(1,$max['enterprises'])."','0','N','0','','notes monitor $i')";
	$db->query($query) or die("PB REQUETE ".$query);	
	$telID=$db->insert_id();
	...
	while (mt_rand(0,100)<$percent['peripherals']){
		$query="INSERT INTO glpi_peripherals VALUES (NULL,'periph of comp $i',NOW(),'contact $i','num $i','$techID','comments $i','".GetRandomString(10)."','".GetRandomString(10)."','$loc','".mt_rand(1,$max['type_peripherals'])."','".mt_rand(1,$max['model_peripherals'])."','brand $i','".mt_rand(1,$max['enterprises'])."','0','N','0','','notes peripherals $i')";
		$db->query($query) or die("PB REQUETE ".$query);
	...
	//	$date=mt_rand(1995,2005)."-".mt_rand(1,12)."-".mt_rand(1,28);
	$query="INSERT INTO glpi_infocoms VALUES (NULL,'$monID','".MONITOR_TYPE."','$date','$date','".mt_rand(12,36)."','infowar mon $monID','".mt_rand(1,$max['enterprises'])."','commande mon $monID','BL mon $monID','immo mon $monID','".mt_rand(0,800)."','".mt_rand(0,500)."','".mt_rand(1,7)."','".mt_rand(1,2)."','".mt_rand(2,5)."','comments mon $monID','facture mon $monID','".mt_rand(1,$max['budget'])."')";
	$db->query($query) or die("PB REQUETE ".$query);
	...
	if (mt_rand(0,100)<=$percent['printer']){
		...
		$typeID=mt_rand(1,$max['type_printers']);
		$modelID=mt_rand(1,$max['model_printers']);
		$query="INSERT INTO glpi_printers VALUES (NULL,'printer of comp $i',NOW(),'contact $i','num $i','$techID','".GetRandomString(10)."','".GetRandomString(10)."','0','0','1','comments $i','".mt_rand(0,64)."','$loc','$domainID','$networkID','$modelID','$typeID','".mt_rand(1,$max['enterprises'])."','0','N','0','','0','notes printers $i')";
		$db->query($query) or die("PB REQUETE ".$query);
		$printID=$db->insert_id();
		add_documents(PRINTER_TYPE,$printID);
		add_contracts(PRINTER_TYPE,$printID);
...
for ($i=0;$i<$max['software'];$i++){
	...
	$techID=mt_rand(1,$max['users_sadmin']+$max['users_admin']);
	$os=mt_rand(1,$max['os']);
	$query="INSERT INTO glpi_software VALUES (NULL,'$name','$version','comments $i','$loc','$techID','$os','N','-1','".mt_rand(1,$max['enterprises'])."','N','0','',NOW(),'notes software $i')";
	$db->query($query) or die("PB REQUETE ".$query);
	$softID=$db->insert_id();
...
for ($i=0;$i<$max['global_peripherals'];$i++){
	$techID=mt_rand(1,$max['users_sadmin']+$max['users_admin']);
	$query="INSERT INTO glpi_peripherals VALUES (NULL,'periph $i',NOW(),'contact $i','num $i','$techID','comments $i','".GetRandomString(10)."','".GetRandomString(10)."','0','".mt_rand(1,$max['type_peripherals'])."','".mt_rand(1,$max['model_peripherals'])."','brand $i','".mt_rand(1,$max['enterprises'])."','1','N','0','','notes peripherals $i')";
	$db->query($query) or die("PB REQUETE ".$query);
	$periphID=$db->insert_id();
	add_documents(PERIPHERAL_TYPE,$periphID);
	add_contracts(PERIPHERAL_TYPE,$periphID);
	...
	for ($j=1;$j<$val;$j++){
		$query="INSERT INTO glpi_connect_wire VALUES (NULL,'$periphID','".($j)."','".PERIPHERAL_TYPE."')";
		$db->query($query) or die("PB REQUETE ".$query);	
	}
}
