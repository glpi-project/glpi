...
function autocompletionTextField($myname,$table,$field,$value='',$size=20,$option=''){
	...
	if ($cfg_features["use_ajax"]&&$cfg_features["ajax_autocompletion"]){
		$rand=mt_rand();
		echo "<input $option id='textfield_$myname$rand' type='text' name='$myname' value=\"".htmlentities($value)."\" size='$size'>\n";
		...
		echo "<input $option type='text' name='$myname' value=\"".htmlentities($value)."\" size=$size'>\n";
	}
}
