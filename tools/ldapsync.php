...
} else if ($nbproc>1 && !(function_exists('pcntl_fork') && function_exists('posix_getpid'))) {
...
function syncEntity ($pid, $data, $server, $prof, $verb, $mail) {
   ...
   if ($entity->getFromDB($id=$data['entities_id'])) {
      ...
      $sql .= ") WHERE glpi_users.authtype = ".Auth::LDAP;
      if ($server>0) {
      ...
      foreach ($req as $row) {
         ...
         if ($result) {
            ...
            echo "  $pid: User '".$row['name']."' problem with LDAP\n";
         }
      }
      ...
      if ($mail) {
         ...
         if ($report) {
            $report  = $LANG['ldap'][1]."\n".
               $LANG['entity'][0] . " : " .$entity->getField('completename') . "\n".
               $LANG['common'][27] . " : " . Html::convDateTime($_SESSION['glpi_currenttime']) . "\n" .
               $report;
            ...
            $mmail->FromName = "GLPI";
            $mmail->Subject = "[GLPI] ".$LANG['setup'][3];
            ...
            if (($mail & 1)
               ...
               if (($mail & 1) && $verb) {
                  echo "  $pid : No address found for emailentity\n";
               }
               $mail = ($mail & 2);
            }
            if (($mail & 2)
               ...
               if (($mail & 2) && $verb) {
                  echo "  $pid : No address found for emailadmin\n";
               }
               $mail = ($mail & 1);
            }
            if ($mail) {
               if ($mmail->Send() && $verb) {
                  echo "  $pid: Report send by email\n";
               }
            } else {
               echo "  $pid: Cannot send report (".
                    $entity->getField('completename').") ".
                    $LANG['mailing'][110]."\n";
            }
         }
      }
