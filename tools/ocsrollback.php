<?php
/*
 * @version $Id: compute_dictionnary.php 6300 2008-01-06 19:12:08Z remi $
 -------------------------------------------------------------------------
...
if (!checkOCSconnection($_GET["server"])) {
...
while ($event=$DB->fetch_array($res)) {
	...
	if ($event["new_value"]>$maxid && 
		...
		if ($event["linked_action"]==10) {// ID Changed
			...
			$sql = "UPDATE glpi_ocs_link SET ocs_id=" . $event["old_value"];
			if (!empty($olddevid)) {
