...
function generate_entity($ID_entity){
	...
	for ($i=0;$i<$MAX['users_sadmin'];$i++){
		$query="INSERT INTO glpi_users VALUES (NULL,'sadmin$i-$ID_entity','',MD5('sadmin$i'),'sadmin$i@tutu.com','tel $i','tel2 $i','mobile $i','sadmin$i name','sadmin$i firstname','0','0','fr_FR','comments $i','-1','-1',NOW(),NOW(),'0')";
		$DB->query($query) or die("PB REQUETE ".$query);
	...
	for ($i=0;$i<$MAX['users_admin'];$i++){
		$query="INSERT INTO glpi_users VALUES (NULL,'admin$i-$ID_entity','',MD5('admin$i'),'admin$i@tutu.com','tel $i','tel2 $i','mobile $i','admin$i name','admin$i firstname','0','0','fr_FR','comments $i','-1','-1',NOW(),NOW(),'0')";
		$DB->query($query) or die("PB REQUETE ".$query);
		$user_id=$DB->insert_id();
	...
	for ($i=0;$i<$MAX['users_normal'];$i++){
		$query="INSERT INTO glpi_users VALUES (NULL,'normal$i-$ID_entity','',MD5('normal$i'),'normal$i@tutu.com','tel $i','tel2 $i','mobile $i','normal$i name','normal$i firstname','0','0','fr_FR','comments $i','-1','-1',NOW(),NOW(),'0')";
		$DB->query($query) or die("PB REQUETE ".$query);
		$user_id=$DB->insert_id();
	...
	for ($i=0;$i<$MAX['users_postonly'];$i++){
		$query="INSERT INTO glpi_users VALUES (NULL,'postonly$i-$ID_entity','',MD5('postonly$i'),'postonly$i@tutu.com','tel $i','tel2 $i','mobile $i','postonly$i name','postonly$i firstname','0','0','fr_FR','comments $i','-1','-1',NOW(),NOW(),'0')";
		$DB->query($query) or die("PB REQUETE ".$query);
		$user_id=$DB->insert_id();
