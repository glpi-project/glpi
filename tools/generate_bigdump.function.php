...
function generate_entity($ID_entity){
	...
	for ($i=0;$i<$MAX['users_sadmin'];$i++){
		$query="INSERT INTO glpi_users VALUES (NULL,'sadmin$i-$ID_entity','',MD5('sadmin$i'),'sadmin$i@tutu.com','tel $i','tel2 $i','mobile $i','sadmin$i name','sadmin$i firstname','0','fr_FR','comments $i','-1','-1',NOW(),NOW(),'0')";
		$DB->query($query) or die("PB REQUETE ".$query);
	...
	for ($i=0;$i<$MAX['users_admin'];$i++){
		$query="INSERT INTO glpi_users VALUES (NULL,'admin$i-$ID_entity','',MD5('admin$i'),'admin$i@tutu.com','tel $i','tel2 $i','mobile $i','admin$i name','admin$i firstname','0','fr_FR','comments $i','-1','-1',NOW(),NOW(),'0')";
		$DB->query($query) or die("PB REQUETE ".$query);
		$user_id=$DB->insert_id();
	...
	for ($i=0;$i<$MAX['users_normal'];$i++){
		$query="INSERT INTO glpi_users VALUES (NULL,'normal$i-$ID_entity','',MD5('normal$i'),'normal$i@tutu.com','tel $i','tel2 $i','mobile $i','normal$i name','normal$i firstname','0','fr_FR','comments $i','-1','-1',NOW(),NOW(),'0')";
		$DB->query($query) or die("PB REQUETE ".$query);
		$user_id=$DB->insert_id();
	...
	for ($i=0;$i<$MAX['users_postonly'];$i++){
		$query="INSERT INTO glpi_users VALUES (NULL,'postonly$i-$ID_entity','',MD5('postonly$i'),'postonly$i@tutu.com','tel $i','tel2 $i','mobile $i','postonly$i name','postonly$i firstname','0','fr_FR','comments $i','-1','-1',NOW(),NOW(),'0')";
		$DB->query($query) or die("PB REQUETE ".$query);
		$user_id=$DB->insert_id();
	...
	for ($i=0;$i<$MAX['type_of_cartridges'];$i++){
		...
		// AJOUT INFOCOMS
		$date=mt_rand(2000,$current_year)."-".mt_rand(1,12)."-".mt_rand(1,28);
		$query="INSERT INTO glpi_infocoms VALUES (NULL,'$cartID','".CARTRIDGE_TYPE."','$date','$date','".mt_rand(12,36)."','infowar cartype $cartID','".mt_rand(1,$MAX['enterprises'])."','commande cartype $cartID','BL cartype $cartID','immo cartype $cartID','".mt_rand(0,5000)."','".mt_rand(0,500)."','".mt_rand(1,7)."','".mt_rand(1,2)."','".mt_rand(2,5)."','comments cartype $cartID','facture cartype $cartID','".mt_rand(1,$MAX['budget'])."','0')";
		$DB->query($query) or die("PB REQUETE ".$query);
